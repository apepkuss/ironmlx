...
| 模型族 | `model_type` | 文本 | 图片 | Chat tools | MTP/辅助 drafter | Prompt Lookup | KV cache |
| --- | --- | :---: | :---: | :---: | :---: | :---: | :---: |
...
Chat tools 仅表示 OpenAI Chat Completions 的结构化函数调用生成与历史回灌；
IronMLX 不执行工具。即使其他模型的模板包含相似标记，也不会被推断为支持。
