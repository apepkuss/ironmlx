...
| 模型族 | `model_type` | 文本 | 图片 | MTP/辅助 drafter | Prompt Lookup | KV cache |
| --- | --- | :---: | :---: | :---: | :---: | :---: |
| Qwen 3.5 Dense / 声明相同类型的 Qwen 3.6 Dense | `qwen3_5` | 是 | 否 | 是 | 是 | 是 |
| Qwen 3.5/3.6 MoE | `qwen3_5_moe` | 是 | 否 | 是 | 是 | 是 |
| Gemma 4 / Gemma 4 Unified | `gemma4`, `gemma4_unified` | 是 | checkpoint 含 `vision_config` 时支持 | 是 | 是 | 是 |
| GLM-4 MoE Lite | `glm4_moe_lite` | 是 | 否 | 否 | 是 | 是 |
| Llama GQA Dense（含兼容的 MiniCPM5-1B） | `llama` | 是 | 否 | 否 | 是 | 是 |
| MiniCPM-V 4.6 | `minicpmv4_6` | 是 | 是 | 否 | 是 | 是 |
| DiffusionGemma | `diffusion_gemma` | 是 | 是 | 否 | 否 | 否 |
