...
3. 通过 `/v1/audio/speech` 提供合成文本和参考音频的 Base64 字符串；可返回完整 WAV，或使用 `stream: true` 与 `response_format: pcm` 接收流式音频。

应用保存资源配置，并在重启时恢复此前加载或固定的模型。首次准备需要联网，完整资源准备后可以离线运行。音频播放由调用方或[独立示例客户端](../../examples/speech-client.swift)完成；Dashboard 提供模型管理与详情。请求参数及完整示例见[语音 API](../audio-speech-api.md)。
