...
[English](../diagnostic-bundle.md)

## 导出与分享

1. 打开 Dashboard 的“日志”页，选择“导出诊断信息”。
2. 在 macOS 保存面板中选择 ZIP 的保存位置。
3. 检查归档内容，再根据需要附到问题报告中；App 不会自动上传。

导出包含系统和版本信息、模型标识与运行状态、故障记录及有限的日志尾部。
不包含模型权重、原始配置、请求正文、Keychain 凭据或完整环境变量；文本还会进行脱敏。
后端离线时仍可导出，收集过程不会启动、重启或停止后端。最终 ZIP 最大为 4 MiB，超出范围的日志会截断。

“故障历史”中的“导出故障记录”只导出当前筛选的 JSON，不是完整诊断 ZIP。
更多说明见[隐私](privacy.md)和[支持](support.md)。

<details>
<summary>技术参考：归档格式、字段及容量限制</summary>
