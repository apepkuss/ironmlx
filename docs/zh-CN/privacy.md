...
- 启用更新的版本会检查 GitHub 托管的更新源，并从更新源提供的地址下载发布文件；自动检查取决于更新器设置，手动检查也会访问网络。
- 打开 Agent 官方配置指南时，浏览器会访问对应文档网站。
- 用户主动启用 LAN 模式时，会在选定的具体局域网地址提供 HTTPS API；
- 推理 API 不会代替客户端抓取远程图片，图片必须作为受控 base64 内容上传。

0.2.0 不包含产品遥测或云端推理上传流程。下载过程会把进度与错误写入本地日志。
模型和更新托管服务仍可能按其自身政策记录客户端 IP、请求和访问 token 的使用情况。
...
事故记录不保存提示词、完整请求体或凭据，日志尾部在持久化与导出时脱敏。故障历史接口只在 App 内嵌桥接中使用，不提供后端或 LAN HTTP 访问。完整诊断 ZIP 的范围见[诊断信息导出](diagnostic-bundle.md)。
