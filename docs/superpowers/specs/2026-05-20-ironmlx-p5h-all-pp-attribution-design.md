...
diagnostic_allowed_by_routing = {
    # Per Codex T4 review P1.2: `first_eval_amortized_cost` (T4.5) is a
    # static OnceLock diagnostic emitted at most once per process via
    # close_p5h_span_diagnostic — must be ALLOWED on Lane-A but is NOT in
    # the REQUIRED presence set (cannot be presence-checked per-request).
    "scheduler": {"sse_write_role_chunk_diagnostic", "first_eval_amortized_cost"},
    "gs_chunked": set(),
}
...
- **Trace context propagation infrastructure** (per § 2.5a "Trace context propagation through call chain"): **Implement exactly the § 2.5a per-thread guard contract — do NOT summarize or re-paraphrase the design here** (per Codex review v8 P2 + v12 P2: every prior round that restated guard semantics drifted from the source within one review round, and v11/v12 specifically caught re-paraphrases that re-introduced unsound Lane-A forwarder guard / closure-wide Lane-B guard). § 2.5a is the single source of truth.
  ...
  - **Fixture validation** (T0a HARD GATE precondition, per Codex review v10 P2 — first-record-only fixture is insufficient because it would pass even when top-level async spans never emit at all):
    ...
    - **Route-aware schema presence check** (per non-aborted sweep request; required sets are split by `span_kind`):
      - If `routing_path == "scheduler"` (Lane A): assert tree span_names ⊇ `{server_request_recv_to_first_content_sse_write, http_parse_render_tokenize, scheduler_admission, model_prefill_forward, first_token_sampling, detok_format_first_content_chunk}` AND diagnostic span_names ⊇ `{sse_write_role_chunk_diagnostic}`. Per Codex v18 P2 #3, Lane-A role chunk is diagnostic because it can overlap with `model_prefill_forward` across threads; it is emitted but NOT included in exclusive child sums. Lane-A diagnostic ALLOWED set (closed-set rejection, distinct from REQUIRED presence set) = `{sse_write_role_chunk_diagnostic, first_eval_amortized_cost}`; `first_eval_amortized_cost` (T4.5) is a one-shot OnceLock diagnostic that fires at most once per process and therefore cannot be REQUIRED per-request — see schema_validator.py `LANE_A_ALLOWED_DIAGNOSTIC` for the source of truth.
      - If `routing_path == "gs_chunked"` (Lane B): assert tree span_names ⊇ `{server_request_recv_to_first_content_sse_write, http_parse_render_tokenize, gs_stream_init_and_chunk_loop, gs_kv_cache_alloc, gs_chunk_N, gs_first_token_sample_dispatch, sse_write_role_chunk, gs_first_token_materialize_and_predispatch, detok_format_first_content_chunk}` AND diagnostic span_names ⊇ `{}`. `gs_kv_cache_alloc`, repeated `gs_chunk_N`, and `gs_first_token_sample_dispatch` are required children of `gs_stream_init_and_chunk_loop`; missing any of them makes the Lane-B top-level coverage gate meaningless. Lane-B tree ALLOWED set (closed-set rejection at line ~254, distinct from REQUIRED presence) = REQUIRED ∪ `{tokenizer_encode}`; `tokenizer_encode` (T4.4) is a retroactive subspan of `http_parse_render_tokenize` emitted in the HTTP handler pre-routing and therefore fires on both lanes — see schema_validator.py `LANE_B_ALLOWED_TREE` for the source of truth.
...
**T4 acceptance criteria** (per Codex T4 review P1.3 — explicit INERT documentation for T4.2):

- T4.1 (`slice_last_and_project_lm_head`): PASS iff every Lane-A PP cell has ≥1 valid record (record_count > 0 AND median_inclusive_us > 0).
- **T4.2 (`mlx_eval_barrier`): INERT today.** The two `try_with_p5h_span_from_current_trace("mlx_eval_barrier", ...)` wraps currently live around the `mlx::transforms::eval` sync sites inside `admit_mid_chunk`, which has no `P5hTraceGuard` on its caller stack — so the spans no-op via the None-tolerant `try_` helper. The Lane-A primary scheduler streaming path has no explicit `mlx::transforms::eval` sync barrier (sync is implicit via `sample_batch.to_vec()`). PASS condition: (a) wraps exist at the 2 admit_mid_chunk eval sites (verified by code-level grep; currently holds) OR (b) records are emitted from a primary-path eval barrier. Today (a) holds; (b) is deferred to **[p5h+1_mid_admit_p5h_ctx_plumbing]**. The T4 harness `mlx_eval_barrier` cell sets `expected_to_emit=false` and `lane="inert"` to reflect this — record_count=0 is the PASS state for this span, NOT a missing-span failure.
- T4.3 (`cache_state_update`): PASS iff every Lane-A PP cell has ≥1 record (CPU-only Arc-share / offset-bump sites legitimately report inclusive_us=0, so the median gate is dropped via `accept_zero_inclusive_us=true`).
- T4.4 (`tokenizer_encode`): PASS iff every observed-lane PP cell (Lane A or Lane B, since it fires pre-routing in the HTTP handler) has ≥1 valid record.
- T4.5 (`first_eval_amortized_cost`): one-shot diagnostic emitted at most once per process via the static OnceLock in `close_p5h_span_diagnostic`. Allowed-but-not-required per § 2.5a schema validator (`LANE_A_ALLOWED_DIAGNOSTIC`); the validator MUST accept Lane-A requests that emit zero `first_eval_amortized_cost` records and MUST accept requests that emit exactly one.
...
| 5 | P5i + P5j candidate ranking with ROI | **Post-P5h+1 sweep** (`b14285a` + `aa20283`) surfaces 6 stable cross-PP candidates: `gather_qmm_gate_up` 20-25% (🔧 KERNEL top-1 every PP), `gather_qmm_down` 10-12% (🔧 KERNEL), `gda_step_1a_in_proj_qkvz` 10-18% (op-level), `gda_step_8_norm_proj` 5-8% (🔧 KERNEL), `fused_sdpa` 6-16% (🔧 KERNEL long-PP O(S²)), `gda_step_7_kernel_dispatch_and_materialize` 5-7% (🔧 KERNEL new T1.5 sub-span). Combined `gather_qmm_*` = 32-35% all PPs. Reported in `docs/p5h+1-ranking-snapshot.md` (committed) + `reports/p5h+1-ranking-snapshot.md` (gitignored). | ✓ **LOCKED PASS (post-P5h+1)** |
| 6 | Target feasibility assessment | **Post-P5h+1 sweep** 4-tier verdict per PP: PP=128 `yes_with_scope_gate` (target +24%, with_kernel 60.9%); PP=512-16384 `no_under_measured_cap` (gap +13% to +63% short to target). All 6 PPs ≠ `data_insufficient`. Probe-mode root inclusive 17-90% larger than production root (forced-eval overhead per spec § 6.5); substep ROI shares accurate; feasibility denominator uses production_root_us baseline. Reported in `/tmp/p5h-t5-verdict.json` + `docs/p5h+1-ranking-snapshot.md` "Feasibility" table. | ✓ **LOCKED PASS (post-P5h+1)** |
...
**P5h ship state (T5 close, 2026-05-23, commit `c4c4642`)**: 7 of 9 gates LOCKED PASS; gates #5 + #6 DEFERRED to P5h+1 due to structural wrapper-dominance instrumentation gaps (Lane A `first_token_sampling` lazy materialization wrapper + Lane B `gs_chunk_N` deep substep deferral). Was a MEASURE-ONLY close-out per plan T5.6.

**P5h ship state (post-P5h+1 close, 2026-05-23)**: **9 of 9 gates LOCKED PASS**. P5h+1 closed wrapper-dominance gap via:
- Lane A: `first_token_sampling` split into `_prepare` + `_materialize_and_sample` sibling pair (commit `d57fbfb`) + per-substep `mlx::transforms::eval` probes gated by `--p5h-measurement-eval-probes` CLI flag (production default OFF).
- Lane B: `LANE_B_ALLOWED_TRY_SPAN_NAMES` extension 3→38 names enabling deep substep emission under `gs_chunk_N` (commits `dfac330` + `b14285a`), with `chunk_idx` schema field + `P5hChunkContextGuard` RAII propagation, + `gda_step_7_kernel_dispatch_and_materialize` sub-span (T1.5 fix `b14285a`) resolving the parent-span self-time residual that initially failed coverage gate.

P5h+1 Close Gate 4/4 PASS verified per `docs/p5h+1-close-out.md` + `docs/p5h+1-ranking-snapshot.md`. P5i + P5j optimization phases may now dispatch on the post-P5h+1 ranking (`gather_qmm_gate_up` 20-25% dominant cross-PP candidate; combined `gather_qmm_{gate_up + down}` 32-35%).

Original `reports/p5h-attribution.md` § 5.4 P5h+1 follow-up list now superseded by `docs/p5h+1-close-out.md` § 8 P5h+2 follow-up list (consolidated: validate_chunk_ancestry cycle hardening + P5hChunkContextGuard.active dead field + emit cost reduction + T2/T3 op-level ablation now data-triggered + T4.2 mid-admit + spec § 1.2 PP=2048 partition addressed via § 7.2.1.5 below).

### 7.2.1.5 P5h+1 close-out cross-reference (2026-05-23)

P5h+1 implementation chain on branch `ironmlx-p5h-perf`:
- `d57fbfb` — T1 Lane A split + per-substep probes + CLI flag (12 files, 8 src + 2 test + 2 Python)
- `dfac330` — T2 Lane B deep substep + chunk_idx schema + try-helper allow-list 3→37 names
- `9f2c06c` — T3 capture harness uses `--p5h-measurement-eval-probes` (1-line)
- `b14285a` — T1.5 `gda_step_7_kernel_dispatch_and_materialize` sub-span (Codex B-lite for coverage gate)
- `aa20283` — T4 ranking snapshot (Close Gate 4/4 PASS)
- (this commit) — T5 close-out doc + spec § 7.2.1 update + memory + § 1.2 PP=2048 reconciliation note

§ 1.2 PP=2048 partition: PP=2048 is the nominal P5j short-PP target per the gap table; actual runtime lane partition derived from observed `routing_path` per `tools/p5h_aggregator/roi_ranking.py:observed_lane_for_pp`. Chat-template overhead (Qwen3 ChatML = 12 tokens per `[project_p5h_t1_findings]`) means `iron-bench --prompt-len 2048` produces server `prompt_tokens=2060 > prefill_chunk_size=2048` and routes through `serve_via_gs_stream` (Lane B). An `iron-bench --prompt-len 2036` invocation is available as a boundary-control sweep targeting exact PP=2048 Lane A behavior but does NOT replace PP=2048 as the primary measurement point. `roi_ranking.py` emits stderr WARN on mismatch (Codex T5-R fixup `1b53dfa`).
