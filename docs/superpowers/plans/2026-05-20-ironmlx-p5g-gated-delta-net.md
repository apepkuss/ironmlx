...
### Repo + rival paths — persist via `/tmp/p5g-env.sh` (sourced by every later block)

Per-block shell sessions don't share env vars (Claude Code may dispatch each bash block as a separate shell invocation). `set -u` then turns any unset `$REPO` / `$RIVALS_DIR` / `$IRONMLX_MOE_MODEL_DIR` into an immediate failure mid-step. Solution: at session start, write the env to `/tmp/p5g-env.sh`; every later block leads with `source /tmp/p5g-env.sh`.
...
QWEN35_MODEL=~/.ironmlx/models/models--mlx-community--Qwen3.5-4B-MLX-4bit/snapshots/$(ls ~/.ironmlx/models/models--mlx-community--Qwen3.5-4B-MLX-4bit/snapshots/ | head -1)

ls "$RIVALS_DIR/omlx/pyproject.toml" > /dev/null && echo "[preflight] RIVALS_DIR=$RIVALS_DIR ok"
ls "$IRONMLX_MOE_MODEL_DIR" > /dev/null && echo "[preflight] IRONMLX_MOE_MODEL_DIR=$IRONMLX_MOE_MODEL_DIR ok"
ls "$QWEN35_MODEL" > /dev/null && echo "[preflight] QWEN35_MODEL=$QWEN35_MODEL ok"

cat > /tmp/p5g-env.sh <<'EOF'
# Generated by P5g plan Pre-flight — sourced by every later bash block.
export REPO="__REPO__"
export RIVALS_DIR="__RIVALS_DIR__"
export IRONMLX_MOE_MODEL_DIR="__IRONMLX_MOE_MODEL_DIR__"
export QWEN35_MODEL="__QWEN35_MODEL__"
export MLX_DIR="${MLX_DIR:-$HOME/.local/mlx}"

# Bounded readiness helper — every shell block that spawns a background server
# (T1/T2/T3 baseline + measured, T4 ironmlx/omlx/mlx-lm) calls this instead of
# an unbounded `until curl` loop. Three guarantees:
#   1. Time-bounded: caps at max_attempts * 5s (default 120 attempts = 10 min).
#   2. Process-alive check: if $pid is no longer running, abort immediately +
#      tail server log (don't block on a dead server's healthz).
#   3. Failure surfaces server log so the operator sees the actual error
#      (model path, port collision, MLX init failure) instead of a generic
#      timeout.
#
# Usage:
#   wait_ready_or_fail <pid> <url> <server_log_path> [max_attempts] [predicate]
#     pid             : server's PID ($!)
#     url             : healthz URL (or /v1/models for omlx, /v1/chat/completions for mlx-lm)
#     server_log_path : file the server's stderr is being written to
#     max_attempts    : default 120 (= 10 min @ 5s/attempt). Bump for huge models.
#     predicate       : default `grep -qE "^(200|404)$"`; override for non-healthz endpoints.
wait_ready_or_fail() {
  local pid="$1"
  local url="$2"
  local log="$3"
  local max_attempts="${4:-120}"
  local predicate_cmd="${5:-default}"
  local i
  for i in $(seq 1 "$max_attempts"); do
    if ! kill -0 "$pid" 2>/dev/null; then
      echo "[error] server PID $pid exited before ready; last 80 lines of $log:" >&2
  ...
  echo "[error] server PID $pid did not become ready at $url after $((max_attempts * 5))s; last 80 lines of $log:" >&2
  tail -80 "$log" 2>/dev/null || echo "(no log file)" >&2
  return 1
}
...
# escape issues that bit earlier when the heredoc tried to expand $REPO inline
# while ALSO needing to emit literal $MLX_DIR for the helper.
sed -i.bak \
    -e "s|__REPO__|$REPO|g" \
    -e "s|__RIVALS_DIR__|$RIVALS_DIR|g" \
    -e "s|__IRONMLX_MOE_MODEL_DIR__|$IRONMLX_MOE_MODEL_DIR|g" \
    -e "s|__QWEN35_MODEL__|$QWEN35_MODEL|g" \
    /tmp/p5g-env.sh
rm -f /tmp/p5g-env.sh.bak

echo "[preflight] /tmp/p5g-env.sh written:"
cat /tmp/p5g-env.sh
```

Expected:
- `pwd` shows the ironmlx-backend repo root.
- `[preflight] ... ok` for all three external paths.
- `/tmp/p5g-env.sh` written; cat output shows the 5 `export` lines + `wait_ready_or_fail` function definition + `export -f` line.

`$RIVALS_DIR` defaults to `/Users/xin/workspace/iron-rivals` (the Boss-environment path); pass `RIVALS_DIR=/other/path` before running this block to override on a different machine layout.
...
- **ablate-t-arr** (Step 7c): instead of constructing `t_arr` from `(&[seq][..], ()).try_into()?` per call (line 621 of current `gated_delta_net.rs`), look up a cached Array keyed by `seq`. `OnceLock<T>` only stores ONE `T`, so the cache structure must be `OnceLock<Mutex<HashMap<i32, Array>>>` (same structure as Step 1.2 C4 promote template — only gating differs).

  **Required:** TWO `#[cfg]` branches so `t_arr` is defined regardless of feature state. The profile branch only changes behavior when `IRONMLX_P5G_PROFILE_MODE=ablate-t-arr` is set; default build is byte-for-byte unchanged.

```rust
// Module-level (gated; only compiled when p5g-profile feature is on):
#[cfg(feature = "p5g-profile")]
static T_ARR_ABLATION_CACHE: std::sync::OnceLock<std::sync::Mutex<std::collections::HashMap<i32, Array>>> =
    std::sync::OnceLock::new();

// In forward_on Step 7c — two cfg branches so t_arr is always defined.
// Use the SAME construction form as the original code: (&[seq][..], ()).try_into()?
// (verified against gated_delta_net.rs:621).
...
Verify default-build identity:

```bash
set -euo pipefail
source /tmp/p5g-env.sh
MLX_DIR=$HOME/.local/mlx cargo build --release -p ironmlx 2>&1 | tee /tmp/p5g-default-build.log
tail -3 /tmp/p5g-default-build.log
# Run sentinel to confirm bit-for-bit identical numerics (no profile feature):
MLX_DIR=$HOME/.local/mlx cargo test -p ironmlx --release --test p5_qwen35_moe_smoke -- --ignored --test-threads=1 2>&1 | tail -10
```

Expected: default build Finished + sentinel argmax=11.
...
set -euo pipefail
source /tmp/p5g-env.sh
# `set -e` + grep no-match = success-here pattern: reverse via `if grep`.
if grep -nE "phase_._.*insert\(pp, 0\.0\)|Phase [CD] require manual|TODO|FIXME|placeholder|stub" \
    "$REPO/ironmlx/tests/p5g_t0_gated_delta_profile.rs"; then
...
The Step 0.14 harness runs Phase A + B + C + Phase D × 3 ablation modes. With per-PP server spawn (4 PPs per phase × 6 phases = 24 spawns), expect ~6-8 hours total wall time. Model load 60s × 24 = ~24 min; iron-bench `--max-tokens 1` per PP × 4 PPs × 6 phases dominates the rest.

```bash
set -euo pipefail
source /tmp/p5g-env.sh

# Defensive: clear any inherited profile mode so Phase A is guaranteed
# no-profile even if the operator's shell has a stray export. Step 0.14's
# `spawn_server()` also calls env_remove, but belt-and-suspenders.
unset IRONMLX_P5G_PROFILE_MODE
...
Verify output — STRICT validation (not just `len(recs) > 0` from prior plan version):
...
for phase_key in ("phase_a_by_pp", "phase_b_by_pp", "phase_c_by_pp"):
    ...
    for pp_str, leaf in d[phase_key].items():
        tps = leaf["pp_tps_median"]
        assert tps and tps > 0, f"{phase_key} PP={pp_str} has bad pp_tps_median={tps}"
        recs = leaf["records"]

        if phase_key == "phase_a_by_pp":
            assert recs == [], f"{phase_key} PP={pp_str} should have no records (no profile mode)"
            continue
...
- [ ] T0 harness ran to completion; `/tmp/p5g-t0-phases.json` exists.
- [ ] Strict validation script passed.
...
RUNS = d["runs"]
WARMUP = d["warmup"]
N_LAYERS = 30  # GDN layers per forward (spec § 1.0)
STEP_NAMES = [
...
# === Phase B aggregation ===
# Per-request total GDN time = sum(elapsed_us across all chunks × N_LAYERS).
# Median across RUNS measured requests.
phase_b_agg = {}
for pp_str, leaf in d["phase_b_by_pp"].items():
    measured = group_by_request(leaf["records"])
    per_request_total_us = []
    per_layer_per_request_us = {layer: [] for layer in range(N_LAYERS)}
    for group in measured:
        # Per-request total = sum across ALL records in this request (all chunks × layers).
        total = sum(float(r.get("elapsed_us", "0")) for r in group)
        per_request_total_us.append(total)
        # Per-layer per-request total — for chunked PP, one layer contributes
        # to one record per chunk; sum across chunks.
        by_layer = {}
        for r in group:
        ...
        for layer, t in by_layer.items():
            per_layer_per_request_us.setdefault(layer, []).append(t)
    ...
    phase_b_agg[pp_str] = {
        ...
        "total_gdn_us_per_request_median": statistics.median(per_request_total_us),
        "per_layer_us_per_request_median": {
            str(l): statistics.median(xs) for l, xs in per_layer_per_request_us.items() if xs
        },
    }
...
# === Phase B-derived occupancy estimate ===
# occupancy = (total_gdn_us_per_request_median) / (request_wall_time_us)
# request_wall_time = prompt_tokens / pp_tps × 1e6 us
phase_b_occupancy = {}
for pp_str, agg in phase_b_agg.items():
    a_tps = phase_a_tps.get(pp_str)
    pp_int = int(pp_str)
    if a_tps and a_tps > 0:
        wall_us = pp_int / a_tps * 1e6
        occ_pct = agg["total_gdn_us_per_request_median"] / wall_us * 100.0
        phase_b_occupancy[pp_str] = {
            "request_wall_us_from_phase_a": wall_us,
            "gdn_total_us": agg["total_gdn_us_per_request_median"],
...
print(json.dumps({
    ...
    "phase_b_occupancy_estimate": phase_b_occupancy,
}, indent=2))
EOF

python3 - <<'EOF'
import json
d = json.load(open("/tmp/p5g-t0-aggregated.json"))
print("=== Phase B per-request GDN total + occupancy estimate ===")
for pp, a in d["phase_b_agg"].items():
    occ = d["phase_b_occupancy_estimate"].get(pp, {})
    print(f"  PP={pp}: {a['total_gdn_us_per_request_median']/1000:.1f} ms over {a['n_chunks_per_request']} chunks (occupancy ~{occ.get('occupancy_pct', 0):.1f}%)")
print("=== Phase C top-3 steps (per PP, per-request totals) ===")
for pp, a in d["phase_c_agg"].items():
    print(f"  PP={pp}: top-3 = {[(n, f'{v/1000:.2f}ms') for n, v in a['ranked_steps'][:3]]}")
print("=== Phase D ablation deltas vs Phase A pp_tps ===")
...
- [ ] Run aggregator. Confirm Phase C top-3 across all 4 PPs identifies a step covered by one of the 3 pre-defined ablation modes:
  - `ablate-compute-g` = step `5_compute_g`
  - `ablate-conv` = steps `2a_concat` / `2b_conv1d_silu` / `2c_update_conv` (chain ablation; cumulative)
  - `ablate-t-arr` = step `7_kernel` (t_arr is a kernel input; cache eliminates per-call construction cost)
- [ ] If Phase C top-3 includes a step NOT mapped to any existing ablation, DECIDE:
  - (a) Add a new `AblateX` variant in Step 0.12 + re-run that one Phase D leg, OR
  - (b) Accept that the 3 pre-defined ablations cover the highest-yield candidates and proceed to T1.
  Document the choice in the T0 report (Step 0.18).
- [ ] If Phase B occupancy estimate < 10% across all PPs, the § 3.4 sanity gate FAILS — STOP, report to Boss before T1.
...
**Layer 2 slowdown vs Layer 1**: <fill>× = Layer 1 phase pp_tps_median ÷ Layer 2 phase pp_tps_median (i.e. Layer 2 wall-time / Layer 1 wall-time). Expect 1.4-1.8× typical for per-op barrier instrumentation per P5e T0 precedent.
...
Apply § 7.1 数学结构: §1 Phase A 实测 wall-time (per PP, derived as
`prompt_len / pp_tps_median`) × §2 GDN occupancy × §4 candidate upper-bound
cut = realistic P5g target. **Do NOT hardcode P5f's 1.111s here** — use the
Phase A wall-time from THIS run so day-to-day GPU variance / model-load
state don't contaminate the ceiling derivation.
...
**Sanity gate (§ 3.4)**: Layer 1 GDN occupancy ≥ 10% (from §2's GDN-occupancy
column, derived from THIS run's Phase A wall-time, not P5f's). Result: <PASS/FAIL>.
If FAIL → Boss decision needed.
...
Expected: all clean + 3/3 sentinel tests PASS. Confirms profile feature is fully gated off in default build (matches Final Acceptance checklist).
...
wait_ready_or_fail "$SERVER_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-t1-start-server.log"
...
SERVER_PID=$!
trap 'kill ${SERVER_PID:-} 2>/dev/null || true' EXIT
wait_ready_or_fail "$SERVER_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-t1-measured-server.log"
...
SERVER_PID=$!
trap 'kill ${SERVER_PID:-} 2>/dev/null || true' EXIT
wait_ready_or_fail "$SERVER_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-t2-start-server.log"
...
SERVER_PID=$!
trap 'kill ${SERVER_PID:-} 2>/dev/null || true' EXIT
wait_ready_or_fail "$SERVER_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-t2-measured-server.log"
...
SERVER_PID=$!
trap 'kill ${SERVER_PID:-} 2>/dev/null || true' EXIT
wait_ready_or_fail "$SERVER_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-t3-start-server.log"
...
wait_ready_or_fail "$IRONMLX_PID" "http://127.0.0.1:8080/healthz" "/tmp/p5g-ironmlx-server.log"
...
( cd "$REPO/scripts/bench-venvs/mlx-lm" && \
  ...
  ) 2> /tmp/p5g-mlx_lm-server.log &
MLXLM_PID=$!
trap 'kill ${MLXLM_PID:-} 2>/dev/null || true' EXIT
# mlx-lm has no /healthz; probe /v1/chat/completions with a 2-token POST until
# the server returns valid `choices`. Override predicate to match the body.
wait_ready_or_fail "$MLXLM_PID" "http://127.0.0.1:8082/v1/chat/completions" "/tmp/p5g-mlx_lm-server.log" 120 \
  'curl -s -X POST http://127.0.0.1:8082/v1/chat/completions -H "Content-Type: application/json" -d "{\"model\":\"default_model\",\"messages\":[{\"role\":\"user\",\"content\":\"hi\"}],\"max_tokens\":2,\"temperature\":0}" 2>/dev/null | grep -q "choices"'
...
### Step 4.5: Aggregate medians

Median aggregation of the 3-way bench output (ironmlx / omlx / mlx-lm). If a close-out scenario later demands p95 (tail latency analysis), add a separate aggregator pass — this script intentionally reports medians only to keep the report focused on the §7.3 ship-metric comparison.
...
- [ ] Create + fill the file. Verify no `<fill>` remaining (note: under `set -e`, `grep` with no match returns 1 — which is the SUCCESS case here. Wrap the check so no-match → exit 0, match → exit 1):

```bash
set -euo pipefail
source /tmp/p5g-env.sh
if grep -n "<fill" "$REPO/reports/p5g-final-results.md"; then
  echo "[error] p5g-final-results.md still contains <fill> placeholders — fill them before commit" >&2
  exit 1
fi
echo "[ok] no <fill> placeholders in reports/p5g-final-results.md"
```
