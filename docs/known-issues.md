...
For changes in a particular release, see its [release notes](release-notes/0.1.0.md).

## Model and API limitations

- Image requests accept JPEG/PNG/WebP base64 content, not remote image URLs.
- The model list can recognize embedding, reranker, ASR and TTS metadata, but the generation backend loads only the LLM/VLM families in [Supported models](supported-models.md).
