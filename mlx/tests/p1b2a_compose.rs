...
fn softmax_along_last_axis_sums_to_one() {
    let x = Array::try_from((&[1.0_f32, 2.0, 3.0][..], &[3][..])).expect("try_from");
    let s = softmax(&x, -1).expect("softmax");
    let v = s.to_vec::<f32>().expect("to_vec");
    let total: f32 = v.iter().sum();
    assert!(
        (total - 1.0).abs() < 1e-6,
        "sum should be ~1.0, got {total}"
    );
...
fn gelu_at_known_points() {
    ...
    // gelu(-1) ≈ -0.1587
    let neg_one = Array::try_from((&[-1.0_f32][..], &[][..])).expect("try_from");
    let g_neg = gelu(&neg_one).expect("gelu").item::<f32>().expect("item");
    assert!(
        (g_neg - (-0.1587)).abs() < 1e-3,
        "gelu(-1) ≈ -0.1587, got {g_neg}"
    );
}
...
fn silu_at_known_points() {
    ...
    let neg_two = Array::try_from((&[-2.0_f32][..], &[][..])).expect("try_from");
    let s_neg = silu(&neg_two).expect("silu").item::<f32>().expect("item");
    assert!(
        (s_neg - (-0.2384)).abs() < 1e-3,
        "silu(-2) ≈ -0.2384, got {s_neg}"
    );
}
