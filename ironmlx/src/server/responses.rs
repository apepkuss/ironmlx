...
use {
    crate::server::chat_format::ChatFunctionCall, crate::server::chat_format::ChatMessage,
    crate::server::chat_format::ChatToolCall, crate::server::chat_format::Content,
    crate::server::chat_format::ContentPart, crate::server::chat_format::ImageUrl,
};
use {
...
use {ironmlx_lm::core::tool_calling::ToolCall, ironmlx_lm::core::tool_calling::ToolDefinition};
...
async fn next_scheduler_event(
    receiver: &mut tokio::sync::mpsc::UnboundedReceiver<
        ironmlx_runtime::core::scheduler::StepEvent,
    >,
    disconnect: Option<&super::api_transport::SseDisconnect>,
) -> anyhow::Result<Option<ironmlx_runtime::core::scheduler::StepEvent>> {
    ...
    for (namespace_index, tool) in tools.iter().enumerate() {
        match tool {
            ...
            ResponseTool::Namespace {
                ...
                ironmlx_lm::core::tool_calling::validate_function_name(namespace)?;
                ...
                for tool in tools {
                    match tool {
                        ResponseNamespaceTool::Function {
                            ...
                            ironmlx_lm::core::tool_calling::validate_function_name(name)?;
                            ...
                            let mut encoding =
                                match ironmlx_lm::core::constrained::validate_tool_schemas(
                                    std::slice::from_ref(&definition),
                                ) {
                                    Ok(()) => {
                                        if *strict == Some(true) {
                                            ironmlx_lm::core::constrained::validate_strict_tool_schema(
                                            &definition,
                                        )?;
                                        }
                            ...
                            if matches!(encoding, NamespaceArgumentEncoding::Structured) {
                                ...
                                if let Err(error) =
                                    ironmlx_lm::core::constrained::validate_tool_schemas(
                ...
                if let Err(error) = ironmlx_lm::core::constrained::validate_tool_schemas(
                    ...
                    ironmlx_lm::core::constrained::validate_tool_schemas(std::slice::from_ref(
                        &collapsed,
                    ))?;
                }
...
#[derive(Debug)]
struct ToolContext {
    dialect: ironmlx_lm::core::tool_calling::ToolDialect,
    definitions: Vec<ToolDefinition>,
    constraint_options: ironmlx_lm::core::constrained::ToolConstraintOptions,
...
struct PreparedResponse {
    #[cfg(test)]
    injected_events: Option<Vec<ironmlx_runtime::core::generation_types::GenerateEvent>>,
        ...
        prepared
            ...
            .map(|constraint_options| ToolContext {
                ...
                output_schema: matches!(
                    ...
                    ironmlx_lm::core::constrained::ToolChoiceConstraint::Auto
                )
...
async fn admit_request<M>(
    state: &AppState<M>,
    request: GenerateRequest,
    #[cfg(test)] injected_events: Option<
        Vec<ironmlx_runtime::core::generation_types::GenerateEvent>,
    >,
...
mod tests {
    ...
    async fn scheduler_eof_after_zero_or_partial_output_is_an_error() {
        for count in [0, 2] {
            ...
            for token in 0..count {
                tx.send(ironmlx_runtime::core::scheduler::StepEvent {
                    id: ironmlx_runtime::core::scheduler::RequestId(1),
                    token,
                    finish_reason: None,
                })
                .unwrap();
    ...
    async fn responses_errors_use_openai_json_and_retry_contracts() {
        let overloaded = crate::server::api_error::ApiError::scheduler_admission(
            ironmlx_runtime::core::scheduler::SchedulerError::QueueFull { capacity: 8 }.into(),
        )
        ...
        let too_large = crate::server::api_error::ApiError::scheduler_admission(
    ...
    fn accepts_hermes_responses_dynamic_tool_bridge_without_changing_chat() {
        ...
        openai::prepare_tool_request(
            &normalized.chat,
            Some(ironmlx_lm::core::tool_calling::ToolDialect::Qwen35),
        )
    ...
    fn qwen38_responses_structured_constraint_preserves_native_reasoning() {
        ...
        let tokenizer =
            ironmlx_lm::core::tokenizer::Tokenizer::from_model_dir(&model_dir).expect("tokenizer");
        let normalized = request(serde_json::json!({
            "model":"local",
