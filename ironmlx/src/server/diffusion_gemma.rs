...
use ironmlx_runtime::core::diffusion_execution::{
    DiffusionGenerateRequest as PreparedRequest, DiffusionGenerationOptions,
};
...
use {
    ironmlx_lm::models::DiffusionGemmaGenerateEvent,
    ironmlx_lm::models::DiffusionGemmaGenerationConfig, ironmlx_lm::models::DiffusionGemmaModel,
};

use super::api_transport::ApiJson;

use ironmlx_runtime::core::diffusion_execution::DiffusionGemmaLaneError;
pub use ironmlx_runtime::core::diffusion_execution::DiffusionGemmaRuntime as DiffusionGemmaAppState;
...
async fn prepare_openai_request(
    ...
    let (image_token_id, _) = ironmlx_lm::core::vision_input::derive_image_token_and_merge(
        &state.vision_input,
        &state.tokenizer,
    );
...
async fn generate_completion(
    ...
    let admitted = state.admit(request.generation).await?;
    admitted
        .spawn(
            move |execution| -> std::result::Result<CompletionParts, String> {
                execution.initialize();
                let events = execution.collect(DiffusionGenerationOptions {
                ...
                execution.record_output_tokens(u64::from(completion.completion_tokens));
                Ok(completion)
            },
        )
        .await
...
async fn openai_stream_completion(
    ...
    let admitted = match state.admit(request).await {
        Ok(guard) => guard,
        Err(err) => {
        ...
        if let Some(context) = tool_context {
            ...
            let generation_result = {
                let mut emit = |event: DiffusionGemmaGenerateEvent| -> Result<bool> {
                    ...
                    if !diffusion_event_is_length_sentinel(&event) {
                        completion_tokens = completion_tokens.saturating_add(1);
                        execution.record_output_tokens(1);
...
use ironmlx_runtime::core::diffusion_execution::build_diffusion_gemma_app_state;
...
mod tests {
    ...
    fn responses_preserve_gemma_native_channel_tokens_without_tools() {
        let native = NativeOutputDecoderConfig {
            dialect: ironmlx_lm::core::native_output::NativeOutputDialect::Gemma,
    ...
    fn diffusion_adapter_rejects_unmapped_typed_output() {
        ...
        let error = collect_tool_parser_events(
            ...
            vec![GeneratedOutputEvent::ImageOutput(
                ironmlx_lm::core::generated_output::ImageArtifact {
                    data: vec![1],
