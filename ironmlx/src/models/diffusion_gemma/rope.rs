...
use super::config::DiffusionGemmaRopeParams;
...
impl RopeOffsets {
    pub fn from_values(values: Vec<i32>) -> Result<Self> {
        if values.is_empty() {
            return Err(anyhow!("DiffusionGemma RopeOffsets cannot be empty"));
        }
...
impl DiffusionGemmaRope {
    pub fn new(dims: i32, traditional: bool, params: &DiffusionGemmaRopeParams) -> Result<Self> {
        match params.rope_type.as_str() {
            ...
            "proportional" => {
                ...
                if rotated_dims <= 0 {
                    return Err(anyhow!(
                        "DiffusionGemma proportional RoPE rotated_dims must be > 0"
                    ));
                }
            ...
            other => Err(anyhow!("DiffusionGemma: unsupported rope_type `{other}`")),
        }
    }
...
fn apply_proportional_rope(
    ...
    if x.ndim() != 4 {
        return Err(anyhow!(
            "DiffusionGemma proportional RoPE expects rank-4 [B,H,S,D], got rank {}",
            x.ndim()
        ));
    }
    ...
    if last < dims {
        return Err(anyhow!(
            "DiffusionGemma proportional RoPE last dim {last} < configured dims {dims}"
        ));
    }
    let half = dims / 2;
