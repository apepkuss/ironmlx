...
use crate::core::speculative::{
    ...
    DraftTokenDistribution, MainCacheRollbackInput, MtpDraftPolicyState, MtpDraftPolicyWindow,
...
struct MtpBatchedFillContext {
    ...
    input_hidden: Array,
    input_token: Array,
    draft_tokens: Vec<Array>,
    mtp_cache_snapshot: MtpCacheSnapshot,
    deferred_tail_commit: Option<MtpDeferredTailCommit>,
}

struct MtpDeviceDraftResult {
    tokens: Vec<Array>,
    cache_snapshot: MtpCacheSnapshot,
}
...
struct SchedulerPromptLookupState {
    cfg: PromptLookupConfig,
    rows: HashMap<usize, SchedulerPromptLookupRowState>,
    stats: PromptLookupStats,
}

struct PromptLookupBatchedFillContext {
    row_idx: usize,
    current_token: u32,
    stop_token_ids: Vec<u32>,
    remaining: usize,
...
struct ImmutablePrefixBlockLookupKey {
    parent: Option<u64>,
    tokens: Box<[u32]>,
}

#[derive(Debug, Clone)]
struct ImmutablePrefixBlockBundle {
    pin: PagedKvBlockOwner,
    handles: Vec<PagedKvImmutableBlockHandle>,
}

#[derive(Debug, Clone)]
struct ImmutablePrefixBlockEntry {
    id: u64,
    chain_hash: u64,
    key: ImmutablePrefixBlockLookupKey,
...
pub struct ImmutablePrefixBlockStats {
    pub blocks: u64,
...
#[derive(Debug)]
struct ImmutablePrefixBlockPool {
    model_id: String,
    block_size: usize,
    max_blocks: usize,
    next_id: u64,
...
impl ImmutablePrefixBlockPool {
    fn new(model_id: String, block_size: i32, max_pages: i32, request_reserve: usize) -> Self {
        let max_blocks = usize::try_from(max_pages)
            .unwrap_or(0)
            .saturating_sub(request_reserve.max(1));
        Self {
            model_id,
            block_size: usize::try_from(block_size).expect("validated positive block size"),
            max_blocks,
            next_id: 0,
            access_clock: 0,
            lookup: HashMap::new(),
...
pub struct RequestOwnedKvStats {
    ...
    pub immutable_prefix: ImmutablePrefixBlockStats,
}
...
/// MTP variant of [`AdmitMidHandle`].
///
/// The ordinary mid-admit path only installs the new row's main-model KV and
/// first token. Qwen MTP also needs an MTP KV cache and final prompt hidden
/// state so the row can join subsequent batched speculative decode steps.
#[doc(hidden)]
pub struct MtpAdmitMidHandle {
    pub request_id: RequestId,
    pub(crate) row_idx: usize,
    pub(crate) prompt_ids: Vec<u32>,
    pub(crate) prompt_len: i32,
...
/// Gemma4 drafter variant of [`AdmitMidHandle`].
///
/// The ordinary mid-admit path only needs final logits to sample the new row's
/// first token. Gemma4 drafter also needs the final prompt hidden state and
/// shared KV state so subsequent speculative windows can draft for the
/// mid-admitted row without falling back to plain decode.
#[doc(hidden)]
pub struct Gemma4DrafterAdmitMidHandle {
    pub request_id: RequestId,
    pub(crate) row_idx: usize,
    pub(crate) prompt_ids: Vec<u32>,
    pub(crate) prompt_len: i32,
...
fn build_mtp_device_verify_input(
    contexts: &[MtpBatchedFillContext],
    cache_row_for_ctx: &[usize],
    batch: usize,
    max_len: usize,
    target: mlx::StreamOrDevice,
    ...
    if contexts.len() != cache_row_for_ctx.len() {
        return Err(anyhow!(
            "build_mtp_device_verify_input: contexts.len()={} != cache_row_for_ctx.len()={}",
            contexts.len(),
            cache_row_for_ctx.len()
        ));
    }

    let mut verify = Array::zeros_on(&[batch as i32, max_len as i32][..], Dtype::Uint32, target)?;
    let mut lens = vec![0_i32; batch];
    for (ctx, &compact_row) in contexts.iter().zip(cache_row_for_ctx.iter()) {
        if compact_row >= batch {
            return Err(anyhow!(
                "build_mtp_device_verify_input: compact row {compact_row} >= batch {batch}"
            ));
        }
        let input_len = ctx.draft_tokens.len() + 1;
        if input_len > max_len {
            return Err(anyhow!(
                "build_mtp_device_verify_input: row {} input_len={input_len} > max_len={max_len}",
                ctx.row_idx
            ));
        }
        let current: Array = (&[ctx.current_token][..], &[1_i32, 1_i32][..]).try_into()?;
        let mut token_steps = Vec::with_capacity(input_len);
        token_steps.push(&current);
        token_steps.extend(ctx.draft_tokens.iter());
        let row = mlx::ops::shape::concatenate_on(&token_steps, 1, target)?;
        verify = mlx::ops::indexing::slice_update_on(
            &verify,
            &row,
            &[compact_row as i32, 0_i32][..],
            &[compact_row as i32 + 1, input_len as i32][..],
            &[1_i32, 1_i32][..],
            target,
        )?;
        lens[compact_row] = input_len as i32;
...
fn pack_greedy_acceptance_on(
    ...
    if dims.len() != 2 || dims[0] <= 0 || dims[1] < 1 {
        return Err(anyhow!(
            "pack_greedy_acceptance_on: expected verify input [B,S>=1], got {dims:?}"
        ));
    }
    if verified_ids.shape().as_slice() != dims {
        return Err(anyhow!(
            "pack_greedy_acceptance_on: verified shape {:?} != verify shape {dims:?}",
            verified_ids.shape().as_slice()
        ));
    }
    let batch = dims[0] as usize;
    let seq = dims[1] as usize;
    if verify_lens.len() != batch || verify_lens.iter().any(|&len| len < 0 || len as usize > seq) {
        return Err(anyhow!(
            "pack_greedy_acceptance_on: invalid verify lengths {verify_lens:?} for shape {dims:?}"
        ));
    }

    if seq == 1 {
        let accepted_len = Array::zeros_on((batch as i32, 1_i32), Dtype::Uint32, target)?;
        return mlx::ops::shape::concatenate_on(&[&accepted_len, verified_ids], 1, target)
            .map_err(Into::into);
    }

    let draft_width = seq - 1;
    let draft_tokens = mlx::ops::indexing::slice_strided_on(
        verify_input,
        &[0_i32, 1_i32][..],
        &[dims[0], dims[1]][..],
        &[1_i32, 1_i32][..],
    ...
    let verified_draft_positions = mlx::ops::indexing::slice_strided_on(
        verified_ids,
        &[0_i32, 0_i32][..],
        &[dims[0], draft_width as i32][..],
        &[1_i32, 1_i32][..],
        target,
    )?;
    let draft_lens = verify_lens
        .iter()
        .map(|&len| len.saturating_sub(1) as u32)
        .collect::<Vec<_>>();
    let draft_lens_arr: Array = (&draft_lens[..], &[batch as i32, 1_i32][..]).try_into()?;
    let positions =
        mlx::ops::constructors::arange_on(0.0, draft_width as f64, 1.0, Dtype::Uint32, target)?
            .reshape_on(&[1_i32, draft_width as i32][..], target)?;
    let valid = positions.less_on(&draft_lens_arr, target)?;
    let matches = draft_tokens.equal_on(&verified_draft_positions, target)?;
    let all_valid_matches = mlx::ops::indexing::where_on(
        &valid,
        &matches,
        &mlx::ops::constructors::ones_on(
            &[batch as i32, draft_width as i32][..],
            Dtype::Bool,
            target,
        )?,
        target,
    )?;
    let accepted_prefix = all_valid_matches
        .astype_on(Dtype::Uint32, target)?
        .cumprod_on(1, false, true, target)?;
    let accepted_mask = mlx::ops::binary::multiply_on(
        &accepted_prefix,
        &valid.astype_on(Dtype::Uint32, target)?,
        target,
    )?;
    let accepted_len = accepted_mask.sum_on(1, true, target)?;
    let corrected = verified_ids.take_along_axis_on(&accepted_len, 1, target)?;
    mlx::ops::shape::concatenate_on(&[&accepted_len, &draft_tokens, &corrected], 1, target)
        .map_err(Into::into)
}

fn resolve_packed_greedy_acceptance(
    packed: &[u32],
    packed_stride: usize,
    compact_row: usize,
    draft_len: usize,
) -> Result<(Vec<u32>, SpeculativeResolution)> {
    let row_start = compact_row
        .checked_mul(packed_stride)
        .ok_or_else(|| anyhow!("resolve_packed_greedy_acceptance: row offset overflow"))?;
    let row = packed
        .get(row_start..row_start + packed_stride)
        .ok_or_else(|| {
            anyhow!(
                "resolve_packed_greedy_acceptance: row {compact_row} outside packed len {} stride {packed_stride}",
                packed.len()
            )
        })?;
    let accepted_draft_len = row[0] as usize;
    if accepted_draft_len > draft_len || draft_len + 2 > packed_stride {
        return Err(anyhow!(
            "resolve_packed_greedy_acceptance: accepted={accepted_draft_len} draft={draft_len} stride={packed_stride}"
        ));
    }
    let draft_tokens = row[1..1 + draft_len].to_vec();
    let corrected = row[packed_stride - 1];
    let mut tokens_to_append = draft_tokens[..accepted_draft_len].to_vec();
...
fn slice_position_ids_row_position(
    ...
    match dims {
        ...
        [1, seq] => {
            if row_idx != 0 || pos as i32 >= *seq {
                return Err(anyhow!(
                    "slice_position_ids_row_position: row={row_idx} pos={pos} out of shape {dims:?}"
                ));
            }
            mlx::ops::indexing::slice_strided_on(
                position_ids,
        ...
        [planes, batch, seq] => {
            if row_idx as i32 >= *batch || pos as i32 >= *seq {
                return Err(anyhow!(
                    "slice_position_ids_row_position: row={row_idx} pos={pos} out of shape {dims:?}"
                ));
            }
            mlx::ops::indexing::slice_strided_on(
                position_ids,
                &[0_i32, row_idx as i32, pos as i32][..],
                &[*planes, row_idx as i32 + 1, pos as i32 + 1][..],
                &[1_i32, 1_i32, 1_i32][..],
                target,
            )
            .map_err(Into::into)
        ...
        _ => Err(anyhow!(
            "slice_position_ids_row_position: expected position_ids [1,1], [1,S], or [P,B,S], got {:?}",
            dims
        )),
    }
}
...
fn build_mtp_batched_position_ids(starts: &[i32], lens: &[i32], max_len: i32) -> Result<Array> {
    if starts.len() != lens.len() {
        return Err(anyhow!(
            "build_mtp_batched_position_ids: starts len {} != lens len {}",
            starts.len(),
            lens.len()
        ));
    }
    if starts.is_empty() {
        return Err(anyhow!("build_mtp_batched_position_ids: batch is empty"));
    }
    if max_len <= 0 {
        return Err(anyhow!(
            "build_mtp_batched_position_ids: max_len must be > 0, got {max_len}"
        ));
    }
    let b = starts.len();
    let s = max_len as usize;
    let mut one_stream = vec![0_i32; b * s];
    for (row, (&start, &len)) in starts.iter().zip(lens.iter()).enumerate() {
        if start < 0 {
            return Err(anyhow!(
                "build_mtp_batched_position_ids: starts[{row}] = {start} must be >= 0"
            ));
        }
        if len < 0 || len > max_len {
            return Err(anyhow!(
                "build_mtp_batched_position_ids: lens[{row}] = {len} out of [0, {max_len}]"
            ));
        }
        for col in 0..len as usize {
            one_stream[row * s + col] = start + col as i32;
        }
    }
    let mut flat = Vec::with_capacity(3 * b * s);
    for _ in 0..3 {
        flat.extend_from_slice(&one_stream);
    }
...
fn pin_full_paged_immutable_block(
    cache: &mut [LayerCache],
    source: PagedKvBlockOwner,
    block_index: i32,
    pin: PagedKvBlockOwner,
) -> Result<Option<ImmutablePrefixBlockBundle>> {
    let mut handles = Vec::new();
    for layer in cache.iter_mut() {
        if let LayerCache::Full(kv) = layer {
            if let Some(handle) = kv.validate_paged_immutable_block_pin(source, block_index, pin)? {
                handles.push(handle);
            }
        }
    }
    ...
    for layer in cache.iter_mut() {
        if let LayerCache::Full(kv) = layer {
            if kv.pin_paged_immutable_block_prevalidated(pin, handles[handle_index]) {
                handle_index += 1;
            }
        }
    }
    anyhow::ensure!(
        ...
        "immutable block pin layer count changed after preflight"
    );
    validate_full_paged_owner_invariants(cache)?;
    Ok(Some(ImmutablePrefixBlockBundle { pin, handles }))
}

fn install_full_paged_immutable_prefix(
    cache: &mut [LayerCache],
    destination: PagedKvBlockOwner,
    bundles: &[ImmutablePrefixBlockBundle],
) -> Result<bool> {
    if bundles.is_empty() {
        return Ok(false);
    }
    let layer_count = bundles[0].handles.len();
    anyhow::ensure!(
        layer_count > 0
            && bundles
                .iter()
                .all(|bundle| bundle.handles.len() == layer_count),
        "immutable prefix bundle layer count mismatch"
    );
    let mut paged_layer = 0_usize;
    for layer in cache.iter_mut() {
        if let LayerCache::Full(kv) = layer {
            let handles = bundles
                .iter()
                .map(|bundle| bundle.handles[paged_layer])
                .collect::<Vec<_>>();
            if kv.validate_paged_immutable_prefix_install(destination, &handles)? {
                paged_layer += 1;
            }
        }
    }
    anyhow::ensure!(
        paged_layer == layer_count,
        "immutable prefix install expected {layer_count} paged layers, found {paged_layer}"
    );
    paged_layer = 0;
    for layer in cache.iter_mut() {
        if let LayerCache::Full(kv) = layer {
            let handles = bundles
                ...
                .map(|bundle| bundle.handles[paged_layer])
                .collect::<Vec<_>>();
            if kv.install_paged_immutable_prefix_prevalidated(destination, &handles) {
                paged_layer += 1;
            }
        }
    }
...
fn immutable_bundle_has_request_refs(
    cache: &[LayerCache],
    bundle: &ImmutablePrefixBlockBundle,
) -> Result<bool> {
    let mut paged_layer = 0_usize;
    ...
    for layer in cache {
        if let LayerCache::Full(kv) = layer {
            let Some(has_request_refs) =
                kv.paged_immutable_handle_has_request_refs(bundle.handles[paged_layer])?
            else {
                continue;
            };
            active |= has_request_refs;
            paged_layer += 1;
        }
    }
    anyhow::ensure!(
        paged_layer == bundle.handles.len(),
        "immutable bundle expected {} paged layers, found {paged_layer}",
        bundle.handles.len()
    );
    Ok(active)
}

fn full_paged_page_bytes(cache: &[LayerCache]) -> usize {
    cache.iter().fold(0_usize, |total, layer| match layer {
        LayerCache::Full(kv) => total.saturating_add(kv.paged_page_bytes().unwrap_or(0)),
        LayerCache::Linear(_) | LayerCache::Mla(_) => total,
    })
}

fn cache_is_fullpaged_only(cache: &[LayerCache]) -> bool {
    !cache.is_empty()
        && cache.iter().all(|layer| match layer {
            LayerCache::Full(kv) => kv.paged().is_some(),
            LayerCache::Linear(_) | LayerCache::Mla(_) => false,
        })
}

fn full_paged_available_unique_pages(cache: &[LayerCache]) -> Option<usize> {
    let mut available = None;
    for layer in cache {
        match layer {
            LayerCache::Full(kv) => {
                let layer_available = kv.paged_available_unique_pages()?;
                available = Some(available.map_or(layer_available, |current: usize| {
                    current.min(layer_available)
                }));
            }
            LayerCache::Linear(_) | LayerCache::Mla(_) => return None,
        }
    }
    available
}

fn additional_full_paged_pages_for_targets(
    cache: &[LayerCache],
    block_size: usize,
    targets: &[(PagedKvBlockOwner, usize)],
) -> Result<usize> {
    targets.iter().try_fold(0_usize, |total, &(owner, target)| {
        let offset = usize::try_from(full_paged_owner_offset(cache, owner)?.unwrap_or(0))
            .map_err(|_| anyhow!("FullPaged owner {owner:?} has a negative offset"))?;
        let current_pages = offset.div_ceil(block_size);
        let target_pages = target.div_ceil(block_size);
        Ok(total.saturating_add(target_pages.saturating_sub(current_pages)))
    })
}
...
fn add_paged_physical_stats(total: &mut PagedKvPhysicalStats, layer: PagedKvPhysicalStats) {
    ...
    total.immutable_pinned_tables = total
        .immutable_pinned_tables
        .saturating_add(layer.immutable_pinned_tables);
    total.immutable_pinned_pages = total
        .immutable_pinned_pages
        .saturating_add(layer.immutable_pinned_pages);
...
fn immutable_prefix_chain_hash(model_id: &str, parent_hash: Option<u64>, tokens: &[u32]) -> u64 {
    let mut hash = 0xcbf2_9ce4_8422_2325_u64;
    update_prefix_fingerprint_str(&mut hash, "ironmlx-fullpaged-block-v1");
    update_prefix_fingerprint_str(&mut hash, model_id);
    update_prefix_fingerprint_hash(&mut hash, &parent_hash.unwrap_or(0).to_le_bytes());
    update_prefix_fingerprint_usize(&mut hash, tokens.len());
    for &token in tokens {
        update_prefix_fingerprint_hash(&mut hash, &token.to_le_bytes());
    }
    hash
}
...
fn evict_one_idle_immutable_leaf(
    ...
    for (_, id) in candidates {
        ...
        if immutable_bundle_has_request_refs(cache, &entry.bundle)? {
        ...
        if let Some(parent) = entry.key.parent {
            let parent = pool
                .entries
                .get_mut(&parent)
                .ok_or_else(|| anyhow!("immutable block {id} parent {parent} is absent"))?;
            parent.child_count = parent
                .child_count
                .checked_sub(1)
                .ok_or_else(|| anyhow!("immutable block {id} parent child count underflow"))?;
        }
...
fn reclaim_idle_immutable_blocks_for_targets(
    pool: &mut ImmutablePrefixBlockPool,
    cache: &mut [LayerCache],
    targets: &[(PagedKvBlockOwner, usize)],
) -> Result<()> {
...
fn validate_immutable_prefix_pool_invariants(
    pool: &ImmutablePrefixBlockPool,
    cache: &[LayerCache],
) -> Result<()> {
    anyhow::ensure!(
        cache_is_fullpaged_only(cache),
        "non-empty immutable prefix pool requires a FullPaged-only cache"
    );
    anyhow::ensure!(
        pool.entries.len() <= pool.max_blocks,
        "immutable prefix pool has {} entries above max {}",
        pool.entries.len(),
        pool.max_blocks
    );
    anyhow::ensure!(
        pool.lookup.len() == pool.entries.len(),
    ...
    let paged_layers = cache
        ...
        .count();
    let mut expected_children = HashMap::<u64, usize>::new();
    for (&id, entry) in &pool.entries {
        anyhow::ensure!(entry.id == id, "immutable prefix entry id mismatch");
        anyhow::ensure!(
            pool.lookup.get(&entry.key) == Some(&id),
            "immutable prefix entry {id} is absent from exact lookup"
        );
        anyhow::ensure!(
            entry.bundle.pin == PagedKvBlockOwner::Immutable(id),
            "immutable prefix entry {id} has mismatched pin {:?}",
            entry.bundle.pin
        );
        anyhow::ensure!(
            entry.bundle.handles.len() == paged_layers,
            "immutable prefix entry {id} has {} handles for {paged_layers} layers",
            entry.bundle.handles.len()
        );
        let parent_hash = match entry.key.parent {
            Some(parent) => {
                let parent_entry = pool.entries.get(&parent).ok_or_else(|| {
                    anyhow!("immutable prefix entry {id} parent {parent} is absent")
                })?;
                *expected_children.entry(parent).or_default() += 1;
                Some(parent_entry.chain_hash)
            }
            None => None,
        };
        anyhow::ensure!(
            entry.chain_hash
                == immutable_prefix_chain_hash(&pool.model_id, parent_hash, &entry.key.tokens),
            "immutable prefix entry {id} chain hash mismatch"
        );
        let mut handle_index = 0_usize;
        for layer in cache {
            if let LayerCache::Full(kv) = layer {
                anyhow::ensure!(
                    kv.validate_paged_immutable_pin_handle(
                        entry.bundle.pin,
                        entry.bundle.handles[handle_index],
                    )?,
                    "immutable prefix entry {id} layer {handle_index} is not FullPaged"
                );
                handle_index += 1;
            }
        }
    ...
    for (&id, entry) in &pool.entries {
        anyhow::ensure!(
            entry.child_count == expected_children.get(&id).copied().unwrap_or(0),
            "immutable prefix entry {id} child count {} is incorrect",
            entry.child_count
        );
    }
    Ok(())
...
fn restore_immutable_prefix_blocks(
    ...
    if pool.max_blocks == 0 || prompt_ids.len() <= 1 {
        return Ok(None);
    }
    let max_blocks = (prompt_ids.len() - 1) / pool.block_size;
    if max_blocks == 0 {
        return Ok(None);
    }
    let mut parent = None;
    let mut bundles = Vec::with_capacity(max_blocks);
    let mut hit_ids = Vec::with_capacity(max_blocks);
    let mut active_hits = 0_u64;
    let mut idle_hits = 0_u64;
    for block_index in 0..max_blocks {
        ...
        let entry = pool
            .entries
            .get(&id)
            .ok_or_else(|| anyhow!("immutable prefix lookup points to absent block {id}"))?;
        let parent_hash = entry
            .key
            .parent
            .and_then(|parent| pool.entries.get(&parent).map(|entry| entry.chain_hash));
        let expected_hash =
            immutable_prefix_chain_hash(&pool.model_id, parent_hash, &entry.key.tokens);
        anyhow::ensure!(
            entry.chain_hash == expected_hash && entry.key == key,
            "immutable prefix block {id} identity mismatch"
        );
        if immutable_bundle_has_request_refs(cache, &entry.bundle)? {
            active_hits = active_hits.saturating_add(1);
        } else {
    ...
    anyhow::ensure!(
        install_full_paged_immutable_prefix(cache, destination, &bundles)?,
        "immutable prefix matched but FullPaged install was unavailable"
    );
    for id in hit_ids {
        pool.touch(id);
    }
    let restored_blocks = bundles.len() as u64;
    pool.stats.restored_blocks = pool.stats.restored_blocks.saturating_add(restored_blocks);
    pool.stats.active_block_hits = pool.stats.active_block_hits.saturating_add(active_hits);
    pool.stats.idle_block_hits = pool.stats.idle_block_hits.saturating_add(idle_hits);
    pool.stats.dedup_saved_bytes = pool
        .stats
        .dedup_saved_bytes
        .saturating_add((full_paged_page_bytes(cache) as u64).saturating_mul(restored_blocks));
    Ok(Some(
        i32::try_from(bundles.len().saturating_mul(pool.block_size))
            .map_err(|_| anyhow!("immutable prefix restored length overflow"))?,
    ))
}
...
fn publish_immutable_prefix_blocks(
    ...
    if pool.max_blocks == 0 || token_ids.len() < pool.block_size {
        return Ok(0);
    }
    let source_offset = full_paged_owner_offset(cache, source)?.unwrap_or(0);
    let source_blocks = usize::try_from(source_offset.max(0)).unwrap_or(0) / pool.block_size;
    let block_count = (token_ids.len() / pool.block_size).min(source_blocks);
    let mut parent = None;
    ...
    for block_index in 0..block_count {
        ...
        let key = ImmutablePrefixBlockLookupKey {
            parent,
            tokens: token_ids[start..end].into(),
        };
        if let Some(&id) = pool.lookup.get(&key) {
            let entry = pool
                .entries
                .get(&id)
                .ok_or_else(|| anyhow!("immutable prefix lookup points to absent block {id}"))?;
        ...
        pool.next_id = pool.next_id.saturating_add(1);
        let pin = PagedKvBlockOwner::Immutable(id);
        let Some(bundle) = pin_full_paged_immutable_block(
            cache,
            source,
            i32::try_from(block_index).map_err(|_| anyhow!("block index overflow"))?,
            pin,
        )?
        else {
            return Ok(published);
        };
        let chain_hash = immutable_prefix_chain_hash(&pool.model_id, parent_hash, &key.tokens);
        pool.access_clock = pool.access_clock.saturating_add(1);
        if let Some(parent_id) = parent {
            let parent_entry = pool
                .entries
                .get_mut(&parent_id)
                .ok_or_else(|| anyhow!("immutable parent {parent_id} is absent"))?;
            parent_entry.child_count = parent_entry.child_count.saturating_add(1);
        }
        pool.lookup.insert(key.clone(), id);
        pool.entries.insert(
...
struct BatchedTextPrefixReplay<'a> {
    ...
    initial_replay_from: &'a [usize],
    prefix_restore_attempted: &'a [bool],
...
fn forward_batched_text_with_paged_prefix<M: Model>(
    ...
    let b = prompt_ids.len();
    anyhow::ensure!(
        initial_replay_from.len() == b,
        "forward_batched_text_with_paged_prefix: initial replay rows {} != batch {b}",
        initial_replay_from.len()
    );
    anyhow::ensure!(
        prefix_restore_attempted.len() == b,
        "forward_batched_text_with_paged_prefix: attempted rows {} != batch {b}",
        prefix_restore_attempted.len()
    );
    let mut replay_from = initial_replay_from.to_vec();
    for group in prefix_restore_groups(prompt_ids, None)? {
        let first_row = group[0];
        if prefix_restore_attempted[first_row] {
            anyhow::ensure!(
                group.iter().all(|&row| prefix_restore_attempted[row]
                    && replay_from[row] == replay_from[first_row]),
                "prefix restore state diverged inside an identical-prompt group"
            );
            continue;
        }
...
pub struct Scheduler<M: Model> {
    ...
    immutable_prefix_blocks: Option<ImmutablePrefixBlockPool>,
    ...
    /// Optional row-scoped Gemma4 assistant-drafter runtime state.
...
impl<M: Model> Scheduler<M> {
    ...
    pub fn enable_paged_prefix_cache(&mut self, config: PagedPrefixCacheConfig) -> Result<()> {
        config.validate()?;
        self.immutable_prefix_blocks = Some(ImmutablePrefixBlockPool::new(
    ...
    pub fn restore_active_kv_request(
        ...
        if let Some(prompt_lookup_row) = parked.prompt_lookup_row.as_ref() {
            ...
            anyhow::ensure!(
                ...
                "active KV restore prompt lookup config mismatch"
            );
        }
        anyhow::ensure!(
            self.slots
    ...
    pub fn request_owned_kv_stats(&self) -> RequestOwnedKvStats {
        ...
        RequestOwnedKvStats {
            ...
            immutable_prefix: self.immutable_prefix_blocks.as_ref().map_or_else(
                ImmutablePrefixBlockStats::default,
                ImmutablePrefixBlockPool::stats,
            ),
        }
    }
    ...
    pub fn fork_request_blocks(
        ...
        let prompt_lookup_fork = if let Some(prompt_lookup) = self.prompt_lookup_state.as_ref() {
            if let Some(destination_row) = prompt_lookup.rows.get(&destination_state.row_idx) {
                anyhow::ensure!(
                    destination_row.pending_tokens.is_empty(),
                    "fork_request_blocks: destination prompt lookup queue is not empty"
                );
            }
            prompt_lookup
                .rows
                .get(&source_state.row_idx)
                .map(|source_row| {
                    anyhow::ensure!(
                        source_row.pending_tokens.is_empty(),
                        "fork_request_blocks: source prompt lookup queue is not empty"
                    );
                    Ok(source_row.clone())
                })
                .transpose()?
        ...
        if let Some(prompt_lookup_row) = prompt_lookup_fork {
            let prompt_lookup = self
                .prompt_lookup_state
                .as_mut()
                .expect("prompt lookup state supplied fork row");
            prompt_lookup
                .rows
                .insert(destination_state.row_idx, prompt_lookup_row);
    ...
    pub fn validate_request_owned_kv_invariants(&self) -> Result<()> {
            ...
            .filter(|pool| !pool.is_empty())
        {
            let cache = self
                .cache
                .as_deref()
                .ok_or_else(|| anyhow!("non-empty immutable prefix pool has no physical cache"))?;
            validate_immutable_prefix_pool_invariants(pool, cache)?;
        }
        Ok(())
    }
    ...
    fn install_initial_cache_for_rows(
        &mut self,
        model: &M,
        rows: &[usize],
        cap: i32,
        ...
        anyhow::ensure!(
            self.cache_rows.is_empty(),
            "initial cache install found live cache rows {:?}",
            self.cache_rows
        );
        let mut new_cache =
            self.make_model_cache_for_rows(model, rows, cap, dtype, turboquant_bits)?;
        let retain_immutable_pool = turboquant_bits.is_none()
            && self
                .immutable_prefix_blocks
                .as_ref()
                .is_some_and(|pool| !pool.is_empty());
        if self.cache.is_some() && !retain_immutable_pool {
            return Err(anyhow!(
                "prefill_admitted: cache already allocated before prefill"
            ));
        }
        if let Some(mut retained_cache) = self.cache.take() {
            if retain_immutable_pool {
                let owners = self.block_owners_for_rows(rows)?;
                anyhow::ensure!(
                    reuse_full_paged_storage(
                        ...
                        &mut retained_cache,
                        &owners,
                        "install_initial_cache_for_rows",
                    )?,
                    "retained immutable prefix pool has no reusable FullPaged storage"
                );
        ...
        self.cache = Some(new_cache);
        self.cache_rows = rows.to_vec();
        Ok(())
    }

    fn discard_retained_immutable_prefix_cache(&mut self) {
    ...
    fn include_fullpaged_owner_in_layout(&mut self, model: &M, row_idx: usize) -> Result<()> {
        anyhow::ensure!(
            !self.cache_rows.contains(&row_idx),
            "FullPaged owner row {row_idx} is already in the execution layout"
        );
        let current_cache = self
            .cache
            .as_ref()
            .ok_or_else(|| anyhow!("FullPaged execution cache is absent"))?;
        anyhow::ensure!(
            cache_is_fullpaged_only(current_cache),
            "immutable in-place layout requires FullPaged-only layers"
        );
        let (cap, dtype) = cache_cap_and_dtype(current_cache)?;
        let mut target_rows = self.cache_rows.clone();
        target_rows.push(row_idx);
        target_rows.sort_unstable();
        let mut new_cache =
            self.make_model_cache_for_rows(model, &target_rows, cap, dtype, None)?;
        let owners = self.block_owners_for_rows(&target_rows)?;
        ...
        anyhow::ensure!(
            reuse_full_paged_storage(
                ...
                &mut old_cache,
                &owners,
                "include_fullpaged_owner_in_layout",
            )?,
            "immutable in-place layout did not reuse FullPaged storage"
        );
        self.cache = Some(new_cache);
        self.cache_rows = target_rows;
        Ok(())
    ...
    fn drop_speculative_row_state(&mut self, row_idx: usize) {
        if let Some(mtp_state) = self.mtp_state.as_mut() {
            mtp_state.rows.remove(&row_idx);
        }
        if let Some(drafter_state) = self.gemma4_drafter_state.as_mut() {
            drafter_state.rows.remove(&row_idx);
        }
    ...
    pub(crate) fn fresh_neural_exact_qualification_regime(
        &self,
        source: NeuralExactSource,
    ) -> Option<NeuralExactRegime> {
        if self.active_count() != 1 {
            return None;
        }
        let state = self.slots.iter().find_map(Option::as_ref)?;
        (!state.finished && state.generated_tokens.is_empty() && !state.sampler.is_pipelinable())
            .then(|| NeuralExactRegime::new(source, state.prompt_ids.len(), state.sampler))
    }

    pub(crate) fn active_batch_finished(&self) -> bool {
        self.active_count() > 0
            && self
                .slots
                .iter()
                .filter_map(Option::as_ref)
                .all(|state| state.finished)
    }

    pub(crate) fn speculative_batch_active_fresh_eligible(&self) -> bool {
        ...
        for state in self.slots.iter().filter_map(|slot| slot.as_ref()) {
            ...
            all_greedy &= state.sampler.is_pipelinable();
        }
        saw_active && (active == 1 || all_greedy)
    }
    ...
    fn emit_pending_token_for_row(
        &mut self,
        row_idx: usize,
        pending_tokens: &mut VecDeque<u32>,
    ) -> Result<StepEvent> {
        let token = pending_tokens
            .pop_front()
            .ok_or_else(|| anyhow!("emit_pending_token_for_row: pending token queue is empty"))?;
        self.emit_token_for_row(row_idx, token)
    }

    fn emit_token_for_row(&mut self, row_idx: usize, token: u32) -> Result<StepEvent> {
        let state = self
            .slots
            .get_mut(row_idx)
            .and_then(Option::as_mut)
            .ok_or_else(|| anyhow!("emit_token_for_row: row {row_idx} slot absent"))?;
        if state.finished {
            return Err(anyhow!(
                "emit_token_for_row: row {row_idx} already finished"
            ));
        }
    ...
    pub fn gemma4_drafter_stats(&self) -> Option<MtpSpeculativeStats> {
    ...
    pub(crate) fn prompt_lookup_qualification_regime(
        ...
        for (&row_idx, row) in &prompt_lookup.rows {
            ...
            if sampler.is_some_and(|current: Sampler| {
                crate::core::prompt_lookup::PromptLookupSamplerFingerprint::from(current)
                    != crate::core::prompt_lookup::PromptLookupSamplerFingerprint::from(
                        slot.sampler,
                    )
            }) {
                return None;
            }
            sampler = Some(slot.sampler);
        ...
        sampler.map(|sampler| {
            PromptLookupQualificationRegime::new(batch_width, context_tokens, sampler)
        })
    }
    ...
    pub fn register_prompt_lookup_request(
        ...
        let row_idx = self
            ...
            .iter()
            .position(|slot| matches!(slot, Some(state) if state.id == id))
            .ok_or_else(|| anyhow!("prompt lookup request {} is not active", id.0))?;
        let state = self.slots[row_idx]
            .as_ref()
            .expect("row_idx found an occupied slot");
        let prompt_lookup = self
        ...
        C: FnMut(&[PromptLookupMtpAcceptedInput], &mut PromptLookupStats) -> Result<()>,
    {
        ...
        let mut prompt_lookup = self
            .prompt_lookup_state
            .take()
            .ok_or_else(|| anyhow!("step_prompt_lookup: scheduler state is absent"))?;
        let result = (|| {
            let active_rows = self
                .slots
                .iter()
    ...
    fn refresh_prompt_lookup_index_stats(state: &mut SchedulerPromptLookupState) {
        ...
        C: FnMut(&[PromptLookupMtpAcceptedInput], &mut PromptLookupStats) -> Result<()>,
    {
        ...
        let active_rows = self
            ...
            .enumerate()
            .filter_map(|(row, slot)| {
                (matches!(slot, Some(state) if !state.finished) && row_states.contains_key(&row))
                    .then_some(row)
            })
            .collect::<Vec<_>>();
        ...
        let verify_result = (|| -> Result<()> {
            let cache_row_for_ctx = contexts
                ...
                .map(|ctx| {
                    self.cache_rows
                        ...
                        .ok_or_else(|| {
                            anyhow!(
                                "fill_prompt_lookup_windows: row {} missing from cache layout {:?}",
                                ctx.row_idx,
                                self.cache_rows
                            )
                        })
            ...
            let max_verify_len = contexts
                .iter()
                .map(|ctx| ctx.draft_tokens.len() + 1)
                .max()
                .unwrap_or(1);
            let mut verify_flat = vec![0_u32; batch * max_verify_len];
            let mut verify_lens = vec![0_i32; batch];
            let mut verify_starts = vec![0_i32; batch];
            for (ctx_idx, ctx) in contexts.iter().enumerate() {
                let compact_row = cache_row_for_ctx[ctx_idx];
                let start = compact_row * max_verify_len;
                verify_flat[start] = ctx.current_token;
                verify_flat[start + 1..start + 1 + ctx.draft_tokens.len()]
                    .copy_from_slice(&ctx.draft_tokens);
                verify_lens[compact_row] = (ctx.draft_tokens.len() + 1) as i32;
            ...
            let verify_logits = if exact_batched_verify {
                ...
                let mut logits_steps = Vec::with_capacity(max_verify_len);
            ...
            let mut resolutions = (0..contexts.len())
                .map(|_| None)
                .collect::<Vec<Option<SpeculativeResolution>>>();
            if all_greedy {
                let verified_ids = mlx::ops::reduction::argmax(&verify_logits, -1, false)?;
                ...
                anyhow::ensure!(
                    ...
                    "fill_prompt_lookup_windows: packed acceptance len {} != {}",
                    packed.len(),
                    batch * packed_stride
                );
                for (ctx_idx, ctx) in contexts.iter().enumerate() {
                    let resolution = resolve_packed_greedy_acceptance(
                        &packed,
                        packed_stride,
                    ...
                    .1;
                    resolutions[ctx_idx] = Some(resolution);
                }
            } else {
                let context_rows = contexts.iter().map(|ctx| ctx.row_idx).collect::<Vec<_>>();
                let compact_prng = self.compact_prng_state_for_rows(&context_rows)?;
                let mut compact_prng_host: Vec<u32> = compact_prng.to_vec()?;
                let vocab = *verify_logits
                    .shape()
                    .as_slice()
                    .last()
                    .ok_or_else(|| anyhow!("prompt lookup verify logits have no vocab axis"))?;
                let sync_start = Instant::now();
                let mut compact_logits_rows = Vec::new();
                let mut compact_samplers = Vec::new();
                let mut compact_histories = Vec::new();
                let mut compact_ranges = Vec::new();
                let mut compact_uniforms = Vec::new();
                for (ctx_idx, ctx) in contexts.iter().enumerate() {
                    let compact_row = cache_row_for_ctx[ctx_idx] as i32;
                    let verify_len = (ctx.draft_tokens.len() + 1) as i32;
                    let row_logits = mlx::ops::indexing::slice(
                        &verify_logits,
                        &[compact_row, 0_i32, 0_i32][..],
                        &[compact_row + 1, verify_len, vocab][..],
                    )?;
                    if ctx.sampler.is_pipelinable() {
                        let verified =
                            mlx::ops::reduction::argmax(&row_logits, -1, false)?.to_vec()?;
                        resolutions[ctx_idx] =
                            Some(resolve_speculative_tokens(&ctx.draft_tokens, &verified)?);
                        continue;
                    }
                    let token_start = compact_samplers.len();
                    compact_samplers.extend(std::iter::repeat_n(&ctx.sampler, verify_len as usize));
                    compact_histories.extend((0..verify_len as usize).map(|position| {
                        let mut position_history = Vec::with_capacity(ctx.history.len() + position);
                        position_history.extend_from_slice(&ctx.history);
                        position_history.extend_from_slice(&ctx.draft_tokens[..position]);
                        position_history
                    }));
                    let key_start = ctx_idx * 2;
                    let mut row_prng: Array =
                        (&compact_prng_host[key_start..key_start + 2], &[2_i32][..]).try_into()?;
                    compact_uniforms.extend(draw_uniforms(&mut row_prng, verify_len as usize)?);
                    let updated_key: Vec<u32> = row_prng.to_vec()?;
                    compact_prng_host[key_start..key_start + 2].copy_from_slice(&updated_key);
                    compact_ranges.push((ctx_idx, token_start, verify_len as usize));
                    compact_logits_rows.push(row_logits.reshape(&[verify_len, vocab][..])?);
                }
                if !compact_logits_rows.is_empty() {
                    let refs = compact_logits_rows.iter().collect::<Vec<_>>();
                    let compact_logits = mlx::ops::shape::concatenate(&refs, 0)?;
                    let compact_history_refs = compact_histories
                        .iter()
                        .map(Vec::as_slice)
                        .collect::<Vec<_>>();
                    let target_tokens = sample_target_tokens_with_uniforms_batch(
                        &compact_samplers,
                        &compact_logits,
                        &compact_history_refs,
                        &compact_uniforms,
                    )?;
                    for &(ctx_idx, token_start, verify_len) in &compact_ranges {
                        let ctx = &contexts[ctx_idx];
                        resolutions[ctx_idx] = Some(resolve_exact_deterministic_target_tokens(
                            &ctx.draft_tokens,
                            &target_tokens[token_start..token_start + verify_len],
                        )?);
                    }
                }
                stats.verify_accept_host_sync_count =
                ...
                prng_commit = Some((
                    context_rows,
                    (&compact_prng_host[..], &[contexts.len() as i32, 2_i32][..]).try_into()?,
                ));
            }
            let mut resolutions = resolutions
                .into_iter()
                .enumerate()
                .map(|(ctx_idx, resolution)| {
                    resolution.ok_or_else(|| {
                        anyhow!("prompt lookup context {ctx_idx} has no speculative resolution")
                    })
                })
                .collect::<Result<Vec<_>>>()?;
            ...
            for (ctx, resolution) in contexts.iter().zip(resolutions.iter_mut()) {
                ...
                stats.rejected_tokens = stats
                    .rejected_tokens
                    .saturating_add(drafted.saturating_sub(accepted));
                if drafted > 0 && accepted == 0 {
                    stats.zero_accept_windows = stats.zero_accept_windows.saturating_add(1);
                }
            ...
            if let Some((rows, compact_prng)) = prng_commit.take() {
                self.scatter_prng_state_from_rows(&rows, &compact_prng)?;
            }
        ...
        M: MtpSpeculativeModel + DenseVlMethods,
    {
        ...
        for &row_idx in &active_rows {
            ...
            let row_cap = state.prompt_ids.len().saturating_add(state.max_new_tokens);
            let row_cap = i32::try_from(row_cap).unwrap_or(i32::MAX);
            final_cap = final_cap.max(row_cap.max(MIN_KV_CACHE_CAP_FOR_GPU_PERF));
        }
        ...
        M: MtpSpeculativeModel,
    {
        ...
        let active_rows: Vec<usize> = self
            ...
            .filter_map(|(row, slot)| {
                (matches!(slot, Some(state) if !state.finished)
                    && mtp_state.rows.contains_key(&row))
                .then_some(row)
            })
            .collect();
        if active_rows.is_empty() {
        ...
        {
            self.mtp_state = Some(mtp_state);
            return self.step_mtp_single_inner(model, mtp);
        }
        ...
        for &row_idx in &active_rows {
            let token = mtp_state
                ...
                .pop_front()
                .ok_or_else(|| {
                    anyhow!("step_mtp_batch: row {row_idx} pending token queue is empty")
                })?;
            events.push(self.emit_token_for_row(row_idx, token)?);
        }

        let rows_needing_postfill = active_rows
            .iter()
            .zip(events.iter())
            .filter_map(|(&row_idx, event)| event.finish_reason.is_none().then_some(row_idx))
            .filter(|row_idx| {
                mtp_state
                    .rows
                    .get(row_idx)
    ...
    fn make_empty_batched_mtp_cache_for_rows(
        &self,
        model: &M,
        mtp: &M::MtpHead,
        contexts: &[MtpBatchedFillContext],
        ...
        M: MtpSpeculativeModel,
    {
        if ctx_indices.is_empty() {
            return Err(anyhow!(
                "make_empty_batched_mtp_cache_for_rows: ctx_indices cannot be empty"
            ));
        }
        let mut cap = 0_i32;
        let mut dtype = None;
        for &ctx_idx in ctx_indices {
            let row_idx = contexts
                .get(ctx_idx)
                .ok_or_else(|| {
                    anyhow!("make_empty_batched_mtp_cache_for_rows: ctx_idx {ctx_idx} out of range")
                })?
                .row_idx;
            let row_state = row_states.get(&row_idx).ok_or_else(|| {
                anyhow!("make_empty_batched_mtp_cache_for_rows: row {row_idx} state absent")
            })?;
            let layer = row_state.mtp_cache.layer(0);
            cap = cap.max(layer.cap());
            if let Some(expected) = dtype {
                if expected != layer.dtype() {
                    return Err(anyhow!(
                        "make_empty_batched_mtp_cache_for_rows: dtype mismatch {:?} != {:?}",
                        expected,
                        layer.dtype()
                    ));
                }
        ...
        let dtype =
            dtype.ok_or_else(|| anyhow!("make_empty_batched_mtp_cache_for_rows: dtype absent"))?;
        model.make_mtp_cache(mtp, ctx_indices.len() as i32, cap, dtype)
    }

    fn make_batched_mtp_cache_from_rows(
        &self,
        model: &M,
        mtp: &M::MtpHead,
        contexts: &[MtpBatchedFillContext],
    ...
    where
        M: MtpSpeculativeModel,
    {
        let mut cache = self.make_empty_batched_mtp_cache_for_rows(
            model,
            mtp,
            contexts,
            row_states,
        ...
        for (compact_idx, &ctx_idx) in ctx_indices.iter().enumerate() {
            let row_idx = contexts
                .get(ctx_idx)
                .ok_or_else(|| {
                    anyhow!("make_batched_mtp_cache_from_rows: ctx_idx {ctx_idx} out of range")
                })?
                .row_idx;
            let row_state = row_states.get(&row_idx).ok_or_else(|| {
                anyhow!("make_batched_mtp_cache_from_rows: row {row_idx} state absent")
            })?;
            cache.adopt_row_from(&row_state.mtp_cache, compact_idx, 0)?;
        }
        Ok(cache)
        ...
        M: MtpSpeculativeModel,
    {
        for ctx in contexts {
            ...
            let mut input_token = ctx.input_token.clone();
        ...
        M: MtpSpeculativeModel,
    {
        ...
        for &row_idx in rows_to_fill {
            let slot = self.slots[row_idx]
                .as_ref()
                .ok_or_else(|| anyhow!("fill_mtp_windows_batched: row {row_idx} slot absent"))?;
            if slot.finished {
                continue;
            }
        ...
        let tail_commit_mode = if Self::should_use_row_cache_mtp_draft(&contexts) {
            ...
            for offset in 0..max_draft_budget {
                ...
                for (compact_idx, &ctx_idx) in active_indices.iter().enumerate() {
                    let next_token = mlx::ops::indexing::slice_strided_on(
                        &sampled_arr,
                        &[compact_idx as i32, 0_i32][..],
                        &[compact_idx as i32 + 1, 1_i32][..],
                        &[1_i32, 1_i32][..],
                        mlx::StreamOrDevice::default(),
                    )?;
        ...
        let verify_result = (|| -> Result<()> {
            let cache_row_for_ctx = contexts
                ...
                .map(|ctx| {
                    self.cache_rows
                        ...
                        .ok_or_else(|| {
                            anyhow!(
                                "fill_mtp_windows_batched: row {} missing from cache layout {:?}",
                                ctx.row_idx,
                                self.cache_rows
                            )
                        })
            ...
            let (verify_arr, verify_lens) = build_mtp_device_verify_input(
                &contexts,
                &cache_row_for_ctx,
                b,
                max_verify_len,
            ...
            let verify_pos_ids = if model.requires_position_ids() {
            ...
            let packed_acceptance = pack_greedy_acceptance_on(
                &verify_arr,
                &verify_lens,
                &verified_ids,
                mlx::StreamOrDevice::default(),
            ...
            let packed_acceptance: Vec<u32> = packed_acceptance.to_vec()?;
            stats.verify_accept_host_sync_count =
                stats.verify_accept_host_sync_count.saturating_add(1);
            add_elapsed_us(&mut stats.verify_accept_host_sync_us, accept_sync_start);
            let packed_stride = max_verify_len + 1;
            if packed_acceptance.len() != b * packed_stride {
                return Err(anyhow!(
                    "fill_mtp_windows_batched: packed acceptance len {} != batch {} * stride {}",
                    packed_acceptance.len(),
                    b,
                    packed_stride
                ));
            }
            ...
            let mut resolved_draft_tokens = Vec::with_capacity(contexts.len());
            for (ctx_idx, ctx) in contexts.iter().enumerate() {
                let compact_row = cache_row_for_ctx[ctx_idx];
                let (draft_tokens, resolution) = resolve_packed_greedy_acceptance(
                    &packed_acceptance,
                    packed_stride,
                    compact_row,
                    ctx.draft_tokens.len(),
                ...
                resolved_draft_tokens.push(draft_tokens);
            ...
            let (accepted_hidden_source, accepted_position_ids_source) =
                if mismatch_rows.is_empty() {
                    ...
                    let accepted_lens = resolutions
                        ...
                        .map(|resolution| resolution.accepted_verify_input_len)
                        .collect::<Vec<_>>();
                    let max_replay_len = accepted_lens.iter().copied().max().unwrap_or(0);
                    if max_replay_len == 0 {
                        return Err(anyhow!(
                            "fill_mtp_windows_batched: accepted replay length cannot be zero"
                        ));
                    }
                    let mut replay_flat = vec![0_u32; b * max_replay_len];
                    let mut replay_lens = vec![0_i32; b];
                    let mut replay_starts = vec![0_i32; b];
                    for (ctx_idx, ctx) in contexts.iter().enumerate() {
                        let compact_row = cache_row_for_ctx[ctx_idx];
                        let accepted_len = accepted_lens[ctx_idx];
                        let verify_len = resolved_draft_tokens[ctx_idx].len() + 1;
                        if accepted_len == 0 || accepted_len > verify_len {
                            return Err(anyhow!(
                                "fill_mtp_windows_batched: row {} accepted_len={} verify_len={}",
                                ctx.row_idx,
                                accepted_len,
                                verify_len
                            ));
                        ...
                        replay_flat[start + 1..start + accepted_len]
                            .copy_from_slice(&resolved_draft_tokens[ctx_idx][..accepted_len - 1]);
                        replay_lens[compact_row] = accepted_len as i32;
                        replay_starts[compact_row] = ctx.verify_start_pos;
                    }
            ...
            let mut full_accept_tail_commits: Vec<(usize, u32, Array, Array)> = Vec::new();
            for (ctx_idx, resolution) in resolutions.into_iter().enumerate() {
                let ctx = &contexts[ctx_idx];
                let compact_row = cache_row_for_ctx[ctx_idx];
                ...
                let mut accepted_input = Vec::with_capacity(accepted_len);
                accepted_input.push(ctx.current_token);
                accepted_input.extend_from_slice(
                    &resolved_draft_tokens[ctx_idx][..accepted_len.saturating_sub(1)],
                );

                let row_state = row_states.get_mut(&ctx.row_idx).ok_or_else(|| {
                    anyhow!(
                        "fill_mtp_windows_batched: row {} MTP state absent after verify",
                        ctx.row_idx
                    )
                })?;
                ...
                if resolution.needs_rollback {
                    ...
                    let tail_position_ids = slice_position_ids_row_position(
                        &accepted_position_ids_source,
                        compact_row,
                        tail_idx,
                        mlx::StreamOrDevice::default(),
                    )?;
                    full_accept_tail_commits.push((
                        ctx_idx,
                        accepted_input[tail_idx],
                        tail_prev_hidden,
                        tail_position_ids,
                    ));
                ...
                match tail_commit_mode {
                    MtpBatchedTailCommitMode::BatchedFromRowCaches => {
                        ...
                        let tail_tokens = full_accept_tail_commits
                            ...
                            .collect::<Vec<_>>();
                        let token_arr: Array =
                            (&tail_tokens[..], &[tail_tokens.len() as i32, 1_i32][..])
                                .try_into()?;
            ...
            for (row_idx, attempted, accepted) in policy_inputs {
                let row_state = row_states.get_mut(&row_idx).ok_or_else(|| {
                    anyhow!(
        ...
        M: MtpSpeculativeModel,
    {
        ...
        let draft_arr = if sampler.is_pipelinable() {
            ...
            Some(mlx::ops::indexing::slice(
                &verify_arr,
                &[0_i32, 1_i32][..],
                &[1_i32, i32::try_from(draft_budget + 1)?][..],
            )?)
        };
        let mtp_cache_snapshot = draft_result.cache_snapshot;
        let verify_len = draft_budget + 1;
    ...
    #[allow(clippy::too_many_arguments)]
    fn draft_mtp_tokens_single_greedy_device(
        &mut self,
        stats: &mut MtpSpeculativeStats,
        row_state: &mut SchedulerMtpRowState,
        model: &M,
    ...
    where
        M: MtpSpeculativeModel,
    {
        let cache_snapshot = row_state.mtp_cache.snapshot();
        let mut tokens = Vec::with_capacity(draft_budget);
        let mut input_hidden = row_state.last_hidden.clone();
        let mut input_token: Array = (&[current_token][..], &[1_i32, 1_i32][..]).try_into()?;
        let start_pos = history_len
            .checked_sub(1)
            .and_then(|value| i32::try_from(value).ok())
            .ok_or_else(|| anyhow!("draft_mtp_tokens_single_greedy_device: invalid history len"))?;
        ...
        M: DenseVlMethods,
    {
        ...
        let mut immutable_replay_from = vec![0_usize; b];
        let mut prefix_restore_attempted = vec![false; b];
        let immutable_fullpaged_enabled = !any_vl
            && turboquant_bits.is_none()
            && self.cache.as_deref().is_some_and(cache_is_fullpaged_only);
        if immutable_fullpaged_enabled {
            let prompt_ids = prefill_rows
                .iter()
                .map(|&slot_row| {
                    self.slots[slot_row]
                        .as_ref()
                        .expect("prefill row remained occupied")
                        .prompt_ids
                        .clone()
                })
                .collect::<Vec<_>>();
            ...
            let restore_inputs = prefill_rows
                .iter()
                .map(|&slot_row| {
                    let state = self.slots[slot_row]
                        .as_ref()
                    ...
                    let owner = self
                        .request_block_tables
                        .get(&state.id)
                        .expect("prefill request block table exists")
                        .owner;
            ...
            if let (Some(pool), Some(cache)) =
                (self.immutable_prefix_blocks.as_mut(), self.cache.as_mut())
            {
                for (cache_row, (owner, prompt_ids)) in restore_inputs.iter().enumerate() {
                    ...
                    if let Some(restored) = restore_immutable_prefix_blocks(
                        ...
                        *owner,
                        ...
                        tracing::debug!(
                            cache_row,
                            restored,
                            "FullPaged immutable prefix block hit"
                        );
                    }
                }
            }
            ...
            let targets = prefill_rows
                ...
                .map(|&slot_row| {
                    ...
                    let owner = self
                        .request_block_tables
                        .get(&state.id)
                        .ok_or_else(|| anyhow!("prefill request block table absent"))?
                        .owner;
                    Ok((owner, state.prompt_ids.len()))
                })
                .collect::<Result<Vec<_>>>()?;
            if let (Some(pool), Some(cache)) =
                (self.immutable_prefix_blocks.as_mut(), self.cache.as_mut())
        ...
        let logits_result: anyhow::Result<Array> = (|| -> anyhow::Result<Array> {
            ...
            let logits = if any_vl {
                ...
                let prompt_ids = &self.slots[prefill_rows[0]]
                    ...
                    .expect("prefill_rows contain only occupied slots")
                    .prompt_ids;
                if immutable_replay_from[0] > 0 {
                    forward_single_text_suffix(
                        model,
                        prefill_cache,
                        prompt_ids,
                        immutable_replay_from[0] as i32,
                        max_len,
                        dummy_position_ids.as_ref(),
                    )?
                } else if let Some(start_pos) = try_restore_paged_prefix_for_prompt(
                    paged_prefix_cache_config.as_ref(),
                    prefix_lru_cache.as_ref(),
                    prefill_cache,
                    prompt_ids,
                ...
                forward_batched_text_with_paged_prefix(
                    ...
                    BatchedTextPrefixReplay {
                        ...
                        initial_replay_from: &immutable_replay_from,
                        prefix_restore_attempted: &prefix_restore_attempted,
                        dummy_position_ids: dummy_position_ids.as_ref(),
                        prefix_cache_config: paged_prefix_cache_config.as_ref(),
                        prefix_lru_cache: prefix_lru_cache.as_ref(),
                    },
        ...
        if immutable_fullpaged_enabled {
            let publish_inputs = prefill_rows
                .iter()
                .map(|&slot_row| {
                    let state = self.slots[slot_row]
                        .as_ref()
            ...
            if let (Some(pool), Some(cache)) =
                (self.immutable_prefix_blocks.as_mut(), self.cache.as_mut())
            {
                for (owner, prompt_ids) in publish_inputs {
                    ...
                    if published > 0 {
                        tracing::debug!(
                            published,
                            request_owner = ?owner,
                            "FullPaged immutable prefix blocks published"
                        );
                    }
                }
            }
        ...
        M: DenseVlMethods,
    {
        ...
        let destination_owner = self
            .request_block_tables
            .get(&id)
            .ok_or_else(|| anyhow!("admit_mid_begin: request block table absent"))?
            .owner;
        ...
        if immutable_restored_start.is_some() {
            if let (Some(pool), Some(cache)) =
                (self.immutable_prefix_blocks.as_mut(), self.cache.as_mut())
            {
                reclaim_idle_immutable_blocks_for_targets(
                    pool,
                    cache,
                    &[(destination_owner, prompt_len_usz)],
                )?;
            }
        }
        let immutable_in_place = immutable_restored_start.is_some();
        let mut temp_cache = if immutable_in_place {
            Vec::new()
        } else {
            self.make_model_cache_for_rows(model, &[row_idx], cap_for_temp, dtype, turboquant_bits)?
        };
        let prefix_restored_start = if let Some(restored) = immutable_restored_start {
            restored
        } else {
            try_restore_paged_prefix_for_prompt(
                self.paged_prefix_cache.as_ref(),
                self.prefix_lru_cache.as_ref(),
                &mut temp_cache,
                &prompt_ids,
    ...
    /// `admit_mid_finalize`.
    fn admit_mid_chunk_immutable_in_place(
        &mut self,
    ...
    {
        anyhow::ensure!(
            !handle.is_vl && handle.temp_cache.is_empty(),
            "in-place immutable mid-admit is text-only and owns no detached cache"
        );
        let previous_rows = self.cache_rows.clone();
        anyhow::ensure!(
            !previous_rows.contains(&handle.row_idx),
            "in-place immutable mid-admit destination is already an execution row"
        );
        let restore_owners = self.block_owners_for_rows(&previous_rows)?;
        let destination_owner = self
            .request_block_tables
            .get(&handle.request_id)
            .ok_or_else(|| anyhow!("in-place immutable mid-admit block table absent"))?
            .owner;
        {
            let cache = self
                .cache
                .as_mut()
                .ok_or_else(|| anyhow!("in-place immutable mid-admit cache absent"))?;
            bind_full_paged_cache_owners(cache, &[destination_owner])?;
        }
        self.cache_rows = vec![handle.row_idx];

        let forward_result = (|| -> Result<Option<Array>> {
            let cache = self
                .cache
                .as_mut()
                .ok_or_else(|| anyhow!("in-place immutable mid-admit cache absent"))?;
        ...
        let restore_result = if restore_owners.is_empty() {
            self.cache_rows.clear();
            Ok(())
        } else {
            let cache = self
                ...
                .as_mut()
                .ok_or_else(|| anyhow!("in-place immutable mid-admit cache absent"))?;
            let result = bind_full_paged_cache_owners(cache, &restore_owners);
            if result.is_ok() {
                self.cache_rows = previous_rows;
            }
            result
        };
        let result_logits = match (forward_result, restore_result) {
            (Ok(logits), Ok(())) => logits,
            (Err(forward), Ok(())) => return Err(forward),
            (Ok(_), Err(restore)) => return Err(restore),
            (Err(forward), Err(restore)) => {
                return Err(forward.context(format!(
                "failed to restore execution rows after in-place immutable mid-admit: {restore:#}"
            )))
            }
        };
        ...
        M: DenseVlMethods,
    {
        ...
        if handle.immutable_in_place {
            drop(prefill_plan);
            return self.admit_mid_chunk_immutable_in_place(
                handle,
                model,
                &input_ids,
                &position_ids,
    ...
    pub fn admit_mid_finalize(
        ...
        if immutable_in_place {
            self.include_fullpaged_owner_in_layout(model, row_idx)?;
            let owner = self
                .request_block_tables
                .get(&id)
    ...
    pub fn admit_mid_begin_mtp(
        &mut self,
        req: GenerateRequest,
        model: &M,
        mtp: &M::MtpHead,
        cfg: MtpSpeculativeConfig,
    ) -> Result<MtpAdmitMidHandle>
    where
        M: DenseVlMethods + MtpSpeculativeModel,
    {
        self.ensure_not_poisoned()?;
        if self.phase != Phase::Decoding {
            return Err(anyhow!(
                "admit_mid_begin_mtp illegal in {:?} phase: only Decoding",
                self.phase
            ));
        }
        let id = self.admit(req)?;
        let row_idx = self
            ...
            .iter()
            .position(|slot| matches!(slot, Some(state) if state.id == id))
            .expect("admit inserted request id");

        match self.admit_mid_begin_mtp_inner(id, row_idx, model, mtp, cfg) {
            Ok(handle) => Ok(handle),
            Err(err) => {
                let _ = self.evict(id);
                Err(err)
        ...
        M: DenseVlMethods + MtpSpeculativeModel,
    {
        let (
            ...
            let state = self.slots[row_idx]
                .as_ref()
                .ok_or_else(|| anyhow!("admit_mid_begin_mtp: row slot absent"))?;
            (
                state.prompt_ids.clone(),
                state.prompt_ids.len(),
                state.max_new_tokens,
        ...
        if prompt_ids.is_empty() {
            return Err(anyhow!("admit_mid_begin_mtp: prompt_ids cannot be empty"));
        }
        if pixel_values.is_none() && image_grid_thw.is_some() {
            return Err(anyhow!(
        ...
        let dtype = {
            let main_cache = self
                .cache
                .as_ref()
                .ok_or_else(|| anyhow!("admit_mid_begin_mtp: main cache absent"))?;
            main_cache
                .iter()
        ...
        let mut chunk_start = 0_i32;
        let mut image_pad_consumed = 0usize;
        let mut mtp_prev_hidden = None;
        let mut last_prompt_hidden = None;
        if let Some((restore_len, restored_last_hidden)) =
            try_restore_paged_prefix_for_prompt_with_mtp(
                self.paged_prefix_cache.as_ref(),
            ...
            )?
        {
            ...
            mtp_prev_hidden = Some(restored_last_hidden.clone());
            if chunk_start >= prompt_len {
                last_prompt_hidden = Some(restored_last_hidden);
            }
        }
    ...
    pub fn admit_mid_chunk_mtp(
        &mut self,
        handle: &mut MtpAdmitMidHandle,
        model: &M,
        mtp: &M::MtpHead,
    ) -> Result<bool>
    where
        M: DenseVlMethods + MtpSpeculativeModel,
    {
        ...
        if handle.chunk_start >= handle.prompt_len {
            anyhow::ensure!(
                handle.last_prompt_hidden.is_some(),
                "admit_mid_chunk_mtp: restored full prompt without last hidden"
            );
            return Ok(true);
        }
        ...
        if chunk_len <= 0 {
            return Err(anyhow!(
                "admit_mid_chunk_mtp: chunk_len <= 0 (chunk_start={}, chunk_end={})",
                handle.chunk_start,
                chunk_end
            ));
        }

        let chunk_ids = &handle.prompt_ids[handle.chunk_start as usize..chunk_end as usize];
        let chunk_pos_ids = if handle.position_ids_required {
            slice_pos_ids_axis2(&handle.position_ids_full, handle.chunk_start, chunk_end)?
        } else {
            handle.position_ids_full.clone()
        };
        ...
        let hidden = if handle.is_vl {
            ...
            let image_rows_start = handle.image_pad_consumed;
            let vision_slice = match handle.vision_embeds_full.as_ref() {
                Some(embeds) if image_tokens > 0 => {
                    let slice = slice_vision_embeds_rows(
                        embeds,
                        image_rows_start,
                        image_rows_start + image_tokens,
                    )?;
                    handle.image_pad_consumed += image_tokens;
                    Some(slice)
                }
                _ => None,
            };
            log_vl_chunk_composition(
                ...
                chunk_ids,
                handle.image_token_id,
                image_rows_start..image_rows_start + image_tokens,
            );
            model.forward_vl_hidden(
                &chunk_arr,
                &chunk_pos_ids,
                None,
                None,
                Some(handle.temp_cache.as_mut_slice()),
                vision_slice.as_ref(),
                handle.image_token_id,
                mlx::StreamOrDevice::default(),
            )?
        } else {
            let chunk_arr: Array = (chunk_ids, &[1_i32, chunk_len][..]).try_into()?;
            model.forward_text_hidden(
                &chunk_arr,
                &chunk_pos_ids,
                None,
        ...
        let prev_hidden = match handle.mtp_prev_hidden.as_ref() {
            Some(hidden) => hidden.clone(),
            None => zero_hidden_like_position(&hidden)?,
        };
        let commit_start = Instant::now();
        ...
        match try_save_paged_prefix_for_prompt_with_mtp(
            ...
            &handle.mtp_cache,
            &chunk_last_hidden,
            &handle.prompt_ids[..new_pos as usize],
            handle.prefix_fingerprint.as_deref(),
        ) {
            Ok(Some(key)) => {
                tracing::debug!("paged SSD prefix cache MTP mid-admit saved: key={key}");
            }
            Ok(None) => {}
            Err(err) => {
    ...
    pub fn admit_mid_finalize_mtp(
        &mut self,
        handle: MtpAdmitMidHandle,
        model: &M,
        mtp: &M::MtpHead,
    ) -> Result<(RequestId, StepEvent)>
    where
        M: DenseVlMethods + MtpSpeculativeModel,
    {
        ...
        let last_prompt_hidden = last_prompt_hidden
            .ok_or_else(|| anyhow!("admit_mid_finalize_mtp: last_prompt_hidden absent"))?;
        self.install_cache_with_temp_row(model, &temp_cache, row_idx)?;

        let (sampler, stop_token_ids, max_new_tokens) = {
            let state = self.slots[row_idx]
                .as_ref()
        ...
        let first_token = *first_tokens
            .first()
            .ok_or_else(|| anyhow!("admit_mid_finalize_mtp produced no first token"))?;
        ...
        let mut mtp_state = self
            .mtp_state
            .take()
            .ok_or_else(|| anyhow!("admit_mid_finalize_mtp: MTP state absent"))?;
        add_mtp_stats(&mut mtp_state.stats, stats);
        if finish_reason.is_none() {
            let row_state = SchedulerMtpRowState {
                mtp_cache,
                pending_tokens: VecDeque::new(),
                last_hidden: last_prompt_hidden,
                deferred_tail_commit: None,
            ...
            let fill_result = (|| {
                let mut temp = self.build_temp_mtp_step_scheduler(
                    ...
                    row_idx,
                    mtp_state.cfg,
                    mtp_state.stats.clone(),
                    row_state,
                )?;
                temp.fill_mtp_window_single(0, model, mtp)?;
                self.install_temp_mtp_step_result(model, row_idx, temp)
            })();
            match fill_result {
                Ok((row_state, stats)) => {
        ...
        for (row_idx, slot) in self.slots.iter_mut().enumerate() {
            if slot.as_ref().is_some_and(|s| s.finished) {
                ...
                if let Some(state) = slot.take() {
                    ...
                    evicted_rows.push(row_idx);
                    self.budget_state.release(state.kv_bytes_admitted);
                }
            }
        }
        for row_idx in evicted_rows {
            self.drop_speculative_row_state(row_idx);
        }
...
impl Scheduler<crate::models::Gemma4Model> {
    pub fn admit_mid_begin_gemma4_drafter(
        ...
        if self.phase != Phase::Decoding {
            return Err(anyhow!(
                "admit_mid_begin_gemma4_drafter illegal in {:?} phase: only Decoding",
                self.phase
            ));
        }
        let id = self.admit(req)?;
        ...
        match self.admit_mid_begin_gemma4_drafter_inner(id, row_idx, model) {
            Ok(handle) => Ok(handle),
            Err(err) => {
                let _ = self.evict(id);
                Err(err)
    ...
    fn admit_mid_begin_gemma4_drafter_inner(
        &mut self,
        id: RequestId,
        row_idx: usize,
        model: &crate::models::Gemma4Model,
        ...
        let (
            ...
            let state = self.slots[row_idx]
                .as_ref()
                .ok_or_else(|| anyhow!("admit_mid_begin_gemma4_drafter: row slot absent"))?;
            (
                state.prompt_ids.clone(),
                state.prompt_ids.len(),
                state.max_new_tokens,
        ...
        if prompt_ids.is_empty() {
            return Err(anyhow!(
                "admit_mid_begin_gemma4_drafter: prompt_ids cannot be empty"
            ));
        }
        if pixel_values.is_none() && image_grid_thw.is_some() {
            return Err(anyhow!(
                "admit_mid_begin_gemma4_drafter: image_grid_thw present but pixel_values is None"
            ));
        }
        ...
        let dtype = {
            let main_cache = self
                .cache
                .as_ref()
                .ok_or_else(|| anyhow!("admit_mid_begin_gemma4_drafter: main cache absent"))?;
            main_cache
                .iter()
                .find_map(|cache| match cache {
                    LayerCache::Full(kv) => Some(kv.dtype()),
        ...
        if let Some(restored) = prefix_cache.try_restore(
            ...
            chunk_start = restored.cached_len;
            image_pad_consumed =
                count_image_pad(&prompt_ids[..chunk_start as usize], image_token_id);
        ...
        let position_ids_full = if is_vl && dummy_position_ids.is_none() {
            let grids = image_grid_thw.as_deref().ok_or_else(|| {
                anyhow!("admit_mid_begin_gemma4_drafter: pixel_values present but image_grid_thw is None")
            })?;
            if model.vl_positions_sequential() {
                Some(build_position_ids(0, prompt_len)?)
            } else {
                let prompt_ids_i32: Vec<i32> = prompt_ids.iter().map(|&id| id as i32).collect();
                Some(build_position_ids_vl(
                    &prompt_ids_i32,
        ...
        let vision_embeds_full = if is_vl {
            let pv = pixel_values
                .as_deref()
                .ok_or_else(|| anyhow!("admit_mid_begin_gemma4_drafter: pixel_values absent"))?;
            let grids = image_grid_thw.as_deref().ok_or_else(|| {
                anyhow!("admit_mid_begin_gemma4_drafter: pixel_values present but image_grid_thw is None")
            })?;
            Some(model.compute_vision_embeds(pv, grids, mlx::StreamOrDevice::default())?)
    ...
    pub fn admit_mid_chunk_gemma4_drafter(
        ...
        if handle.chunk_start >= handle.prompt_len {
            anyhow::ensure!(
                handle.last_prompt_hidden.is_some() && handle.last_shared_kv.is_some(),
                "admit_mid_chunk_gemma4_drafter: restored full prompt without drafter state"
            );
            return Ok(true);
        }
        ...
        if chunk_len <= 0 {
            return Err(anyhow!(
                "admit_mid_chunk_gemma4_drafter: chunk_len <= 0 (chunk_start={}, chunk_end={})",
                handle.chunk_start,
                chunk_end
            ));
        }
        ...
        let chunk_ids = &handle.prompt_ids[handle.chunk_start as usize..chunk_end as usize];
        let chunk_arr: Array = (chunk_ids, &[1_i32, chunk_len][..]).try_into()?;
        let chunk_pos_ids = match (
            handle.dummy_position_ids.as_ref(),
            handle.position_ids_full.as_ref(),
        ) {
            (Some(dummy), _) => dummy.clone(),
            (None, Some(full)) => slice_pos_ids_axis2(full, handle.chunk_start, chunk_end)?,
        ...
        match prefix_cache.try_save(
            ...
            Ok(Some(key)) => {
                tracing::debug!("paged SSD prefix cache Gemma4 drafter mid-admit saved: key={key}");
            }
            Ok(None) => {}
            Err(err) => {
                tracing::warn!(
                    "paged SSD prefix cache Gemma4 drafter mid-admit save skipped: {err:#}"
                );
            }
        }
        self.refresh_active_kv_residency_stats();
    ...
    pub fn admit_mid_finalize_gemma4_drafter(
        ...
        let last_prompt_hidden = last_prompt_hidden.ok_or_else(|| {
            anyhow!("admit_mid_finalize_gemma4_drafter: last_prompt_hidden absent")
        })?;
        let shared_kv = last_shared_kv
            .ok_or_else(|| anyhow!("admit_mid_finalize_gemma4_drafter: shared KV absent"))?;
        ...
        let first_token = *first_tokens
            .first()
            .ok_or_else(|| anyhow!("admit_mid_finalize_gemma4_drafter produced no first token"))?;
        ...
        let mut drafter_state = self
            .gemma4_drafter_state
            .take()
            .ok_or_else(|| anyhow!("admit_mid_finalize_gemma4_drafter: drafter state absent"))?;
        add_mtp_stats(&mut drafter_state.stats, stats);
        if finish_reason.is_none() {
            drafter_state.rows.insert(
                row_idx,
    ...
    fn prefill_admitted_gemma4_drafter_batch_inner(
        ...
        match self.phase {
            ...
            Phase::Decoding | Phase::Finished => {
                return Err(anyhow!(
                    "prefill_admitted_gemma4_drafter_batch illegal in {:?} phase: call evict_all first",
                    self.phase
                ));
            }
        }
        ...
        if self.cache.is_some() {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_batch: cache already allocated before prefill"
            ));
        }
        ...
        if active_rows.is_empty() {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_batch requires at least one admitted request"
            ));
        }
        if active_rows.len() == 1 {
            return self
        ...
        for &row_idx in &active_rows {
            ...
            if state.finished || !state.generated_tokens.is_empty() {
                return Err(anyhow!(
                    "prefill_admitted_gemma4_drafter_batch requires fresh admitted rows"
                ));
            }
            if state.prompt_ids.is_empty() {
                return Err(anyhow!(
                    "prefill_admitted_gemma4_drafter_batch: prompt_ids cannot be empty"
                ));
            }
            MtpSpeculativeConfig::new(cfg.max_draft_tokens, state.sampler)?;
        }
        ...
        for &row_idx in &active_rows {
            ...
            let temp_cache = temp.cache.take().ok_or_else(|| {
                anyhow!("prefill_admitted_gemma4_drafter_batch row {row_idx}: temp cache absent")
            })?;
            ...
            let mut temp_state = temp.gemma4_drafter_state.take().ok_or_else(|| {
                anyhow!(
                    "prefill_admitted_gemma4_drafter_batch row {row_idx}: temp drafter state absent"
                )
            })?;
            add_mtp_stats(&mut stats, temp_state.stats);
            let row_state = temp_state.rows.remove(&0).ok_or_else(|| {
                anyhow!(
                    "prefill_admitted_gemma4_drafter_batch row {row_idx}: temp row state absent"
                )
            })?;
            row_states.insert(row_idx, row_state);
            let temp_slot = temp.slots[0].as_ref().ok_or_else(|| {
                anyhow!("prefill_admitted_gemma4_drafter_batch row {row_idx}: temp slot absent")
            })?;
            let slot = self.slots[row_idx]
                .as_mut()
                .expect("active row implies slot is Some");
        ...
        for (dst_row, &slot_row) in active_rows.iter().enumerate() {
            let (_, temp_cache) = temp_rows
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "prefill_admitted_gemma4_drafter_batch: missing temp cache for slot row {slot_row}"
                    )
                })?;
            adopt_cache_row_layers(
                &mut final_cache,
                ...
                dst_row,
                0,
                "prefill_admitted_gemma4_drafter_batch",
            )?;
        }
    ...
    fn prefill_admitted_gemma4_drafter_single(
        ...
        match self.phase {
            ...
            Phase::Decoding | Phase::Finished => {
                return Err(anyhow!(
                    "prefill_admitted_gemma4_drafter_single illegal in {:?} phase: call evict_all first",
                    self.phase
                ));
            }
        }
        ...
        if active_rows.len() != 1 {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_single requires exactly one admitted request, got {}",
                active_rows.len()
            ));
        }
        let row_idx = active_rows[0];
        ...
        if prompt_ids.is_empty() {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_single: prompt_ids cannot be empty"
            ));
        }
        if pixel_values.is_none() && image_grid_thw.is_some() {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_single: image_grid_thw present but pixel_values is None"
            ));
        }
        MtpSpeculativeConfig::new(cfg.max_draft_tokens, sampler)?;
        ...
        if self.cache.is_some() {
            return Err(anyhow!(
                "prefill_admitted_gemma4_drafter_single: cache already allocated before prefill"
            ));
        }
        let turboquant_bits = self.kv_cache_turboquant_bits_for_rows(&[row_idx])?;
        self.cache =
        ...
        let position_ids_full = if is_vl && dummy_position_ids.is_none() {
            let grids = image_grid_thw.as_deref().ok_or_else(|| {
                anyhow!(
                    "prefill_admitted_gemma4_drafter_single: pixel_values present but image_grid_thw is None"
                )
            })?;
            if model.vl_positions_sequential() {
        ...
        if let Some(restored) = {
            let cache = self
                .cache
                .as_mut()
                .ok_or_else(|| anyhow!("prefill_admitted_gemma4_drafter_single: cache absent"))?;
            prefix_cache.try_restore(
                model,
                cache.as_mut_slice(),
                &prompt_ids,
                prefix_fingerprint.as_deref(),
            ...
            image_pad_consumed = count_image_pad(&prompt_ids[..pos as usize], image_token_id);
            last_prompt_hidden = Some(restored.last_hidden);
            last_shared_kv = Some(restored.shared_kv);
            self.refresh_active_kv_residency_stats();
        }
        ...
        while pos < prompt_len_i32 {
            ...
            let forward_start = Instant::now();
            let out = if is_vl {
                let image_tokens = count_image_pad(chunk_ids, image_token_id);
                if image_tokens > 0 && vision_embeds_full.is_none() {
                    let pv = pixel_values.as_deref().ok_or_else(|| {
                        anyhow!(
                            "prefill_admitted_gemma4_drafter_single: image tokens without pixel_values"
                        )
                    })?;
                    let grids = image_grid_thw.as_deref().ok_or_else(|| {
                        anyhow!(
                            "prefill_admitted_gemma4_drafter_single: image tokens without image_grid_thw"
                        )
                    })?;
                    vision_embeds_full = Some(model.compute_vision_embeds(
                        pv,
                        grids,
                        mlx::StreamOrDevice::default(),
                    )?);
                ...
                let vision_slice = match vision_embeds_full.as_ref() {
                    Some(embeds) if image_tokens > 0 => Some(slice_vision_embeds_rows(
                        embeds,
                        image_pad_consumed,
                ...
                image_pad_consumed += image_tokens;
                let cache = self.cache.as_mut().ok_or_else(|| {
                    anyhow!("prefill_admitted_gemma4_drafter_single: cache absent")
                })?;
                model.forward_vl_hidden_with_shared_kv_on(
                    &chunk_arr,
                    &chunk_pos_ids,
            ...
            if let Some(cache) = self.cache.as_ref() {
                match prefix_cache.try_save(
                    ...
                    Err(err) => {
                        tracing::warn!(
                            "paged SSD prefix cache Gemma4 drafter save skipped: {err:#}"
                        );
                    }
                }
            }
        ...
        let last_prompt_hidden = last_prompt_hidden.ok_or_else(|| {
            anyhow!("prefill_admitted_gemma4_drafter_single produced no prompt hidden")
        })?;
        let shared_kv = last_shared_kv.ok_or_else(|| {
            anyhow!("prefill_admitted_gemma4_drafter_single produced no shared KV")
        })?;
        let projection_start = Instant::now();
        let first_logits =
            model.project_hidden_on(&last_prompt_hidden, mlx::StreamOrDevice::default())?;
        ...
        let first_token = *first_tokens.first().ok_or_else(|| {
            anyhow!("prefill_admitted_gemma4_drafter_single produced no first token")
        })?;
    ...
    fn step_gemma4_drafter_batch_inner(
        ...
        if self.phase != Phase::Decoding {
            return Err(anyhow!(
                "step_gemma4_drafter_batch illegal in {:?} phase: call prefill_admitted_gemma4_drafter_batch first",
                self.phase
            ));
        }
        let mut drafter_state = self
        ...
        let active_rows: Vec<usize> = self
            ...
            .filter_map(|(row, slot)| {
                (matches!(slot, Some(state) if !state.finished)
                    && drafter_state.rows.contains_key(&row))
                .then_some(row)
            })
            .collect();
        if active_rows.is_empty() {
    ...
    fn step_gemma4_drafter_single(
        ...
        if self.phase != Phase::Decoding {
            return Err(anyhow!(
                "step_gemma4_drafter_single illegal in {:?} phase: call prefill_admitted_gemma4_drafter_single first",
                self.phase
            ));
        }
        let active_rows: Vec<usize> = self
        ...
        if active_rows.len() != 1 {
            return Err(anyhow!(
                "step_gemma4_drafter_single requires exactly one unfinished row, got {}",
                active_rows.len()
            ));
        }
        let row_idx = active_rows[0];
        ...
        if self
            .gemma4_drafter_state
            .as_ref()
            .ok_or_else(|| anyhow!("step_gemma4_drafter_single: drafter state absent"))?
            .rows
            .get(&row_idx)
            .ok_or_else(|| anyhow!("step_gemma4_drafter_single: row state absent"))?
            .pending_tokens
            .is_empty()
        {
        ...
        let token = {
            ...
            drafter_state
                ...
                .pop_front()
                .ok_or_else(|| {
                    anyhow!("step_gemma4_drafter_single: pending token queue is empty")
                })?
        };
    ...
    fn fill_gemma4_drafter_window_single(
        ...
        let mut drafter_state = self
            .gemma4_drafter_state
            .take()
            .ok_or_else(|| anyhow!("fill_gemma4_drafter_window_single: drafter state absent"))?;
        let mut row_state = drafter_state.rows.remove(&row_idx).ok_or_else(|| {
            anyhow!("fill_gemma4_drafter_window_single: row {row_idx} state absent")
        })?;
    ...
    fn fill_gemma4_drafter_window_single_with_state(
        ...
        let current_token = *generated_tokens
            .last()
            .ok_or_else(|| anyhow!("fill_gemma4_drafter_window_single: no current token"))?;
        let mut history = Vec::with_capacity(prompt_ids.len() + generated_tokens.len());
        history.extend_from_slice(&prompt_ids);
        history.extend_from_slice(&generated_tokens);
    ...
    #[allow(clippy::too_many_arguments)]
    fn draft_gemma4_drafter_tokens_single(
        &mut self,
        stats: &mut MtpSpeculativeStats,
        row_state: &mut SchedulerGemma4DrafterRowState,
        model: &crate::models::Gemma4Model,
        ...
        for &draft_uniform in draft_uniforms.iter().take(draft_budget) {
            ...
            let token_embed = model.embed_on(&token_arr, mlx::StreamOrDevice::default())?;
            let inputs_embeds = mlx::ops::shape::concatenate_on(
                &[&token_embed, &input_hidden],
                2,
                mlx::StreamOrDevice::default(),
            )?;
            let draft_forward_start = Instant::now();
            let output = drafter.forward_on(
                &inputs_embeds,
                &row_state.shared_kv,
                draft_position,
                kv_valid_len,
    ...
    fn temp_gemma4_drafter_scheduler_for_row(
        ...
        let temp_id = temp.admit(generate_request_from_state(state)?)?;
        {
            let temp_state = temp.slots[0].as_mut().ok_or_else(|| {
                anyhow!("temp_gemma4_drafter_scheduler_for_row: temp slot absent")
            })?;
            temp_state.id = state.id;
            temp_state.generated_tokens = state.generated_tokens.clone();
            temp_state.real_len = state.real_len;
...
mod tests {
    ...
    fn device_greedy_acceptance_packs_mismatch_and_bonus_tokens() {
        let verify_input: Array = (&[9_u32, 10, 11, 12, 19, 20, 21, 0][..], &[2_i32, 4_i32][..])
            .try_into()
            .expect("verify input");
        let verified_ids: Array = (
            &[10_u32, 99, 12, 77, 20, 21, 22, 0][..],
            &[2_i32, 4_i32][..],
        )
            .try_into()
            .expect("verified ids");
        ...
        .expect("pack acceptance")
        .to_vec::<u32>()
        .expect("sync packed acceptance");
        assert_eq!(packed, vec![1, 10, 11, 12, 99, 2, 20, 21, 0, 22]);

        let (draft0, resolution0) =
            resolve_packed_greedy_acceptance(&packed, 5, 0, 3).expect("resolve row 0");
        assert_eq!(draft0, vec![10, 11, 12]);
        assert_eq!(resolution0.accepted_draft_len, 1);
        assert_eq!(resolution0.tokens_to_append, vec![10, 99]);
        assert!(resolution0.needs_rollback);

        let (draft1, resolution1) =
            resolve_packed_greedy_acceptance(&packed, 5, 1, 2).expect("resolve row 1");
        assert_eq!(draft1, vec![20, 21]);
        assert_eq!(resolution1.accepted_draft_len, 2);
        assert_eq!(resolution1.tokens_to_append, vec![20, 21, 22]);
        assert!(!resolution1.needs_rollback);
    }
    ...
    impl ScriptedMtpSchedulerModel {
        ...
        fn clear_mtp_hidden_trace(&self) {
            self.mtp_hidden_seq_lens.lock().unwrap().clear();
    ...
    impl crate::core::model::Model for ScriptedMtpSchedulerModel {
        ...
        fn project_hidden_on(
            ...
            let batch = hidden_dims[0] as usize;
            ...
            *calls += 1;
            ...
            if batch == 1 {
                ...
                fake_logits_for_batch_token_sequences(batch, seq, &tokens)
            }
        }
    ...
    fn assert_mtp_prefill_only_cache_commit(stats: &MtpSpeculativeStats) {
        assert_eq!(
            stats.mtp_decode_cache_commit_us, 0,
            "prefill should not contribute to decode MTP cache commit cost"
        );
        assert_eq!(
            stats.mtp_cache_commit_us, stats.mtp_prefill_cache_commit_us,
            "prefill-only runs should attribute all MTP cache commit cost to prefill"
        );
    }
    ...
    fn mtp_prefill_uses_paged_ssd_prefix_cache_on_exact_hit() {
        ...
        assert_mtp_prefill_only_cache_commit(&warm.mtp_stats().expect("warm MTP stats"));
    ...
    fn mtp_batch_eligibility_accepts_single_text_greedy_request() {
        ...
        assert!(scheduler.speculative_batch_active_fresh_eligible());
    }

    #[test]
    fn speculative_batch_eligibility_accepts_sampled_b1_and_rejects_sampled_b2() {
        ...
        assert!(vl_scheduler.speculative_batch_active_fresh_eligible());
        ...
        assert!(sampling_scheduler.speculative_batch_active_fresh_eligible());

        let mut sampled_b2_scheduler = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            16,
            crate::core::memory_budget::test_meta_qwen35(),
        )
        ...
        for seed in [7, 11] {
            let mut sampled = mtp_req(vec![1, 2], 4);
            sampled.sampler = Sampler::greedy().with_temperature(0.7).with_seed(seed);
            sampled_b2_scheduler.admit(sampled).expect("admit sampled");
        }
        assert!(!sampled_b2_scheduler.speculative_batch_active_fresh_eligible());
    }
    ...
    fn mtp_step_emits_one_pending_token_per_call() {
        ...
        assert_eq!(stats.verify_accept_host_sync_count, stats.windows);
    }

    #[test]
    #[serial(mlx_metal)]
    fn mtp_sampled_b1_runs_exact_acceptance_and_commits_cache() {
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            1,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        .expect("scheduler startup");
        let sampler = Sampler::greedy().with_temperature(0.7).with_seed(19);
        let mut request = mtp_req(vec![1, 2], 4);
        request.sampler = sampler;
        let id = s.admit(request).expect("admit sampled request");
        let model = ScriptedMtpSchedulerModel::new(3, vec![4, 5], vec![vec![4, 5, 6]]);
        let cfg = MtpSpeculativeConfig::new(2, sampler).expect("sampled mtp cfg");

        let first = s
            .prefill_admitted_mtp_single(&model, &FakeMtpHead, cfg)
            .expect("sampled mtp prefill");
        assert_eq!(
            first,
            vec![StepEvent {
                id,
        ...
        assert_eq!(
            mtp_cache_offset(&s),
            5,
            "full exact acceptance must commit the sampled MTP tail"
        );

        let stats = s.mtp_stats().expect("sampled mtp stats");
        assert_eq!(stats.windows, 1);
        assert_eq!(stats.drafted_tokens, 2);
        assert_eq!(stats.accepted_draft_tokens, 2);
        assert_eq!(stats.exact_sampling_windows, 1);
        assert_eq!(stats.exact_acceptance_draws, 2);
        assert_eq!(stats.exact_residual_corrections, 0);
        assert_eq!(stats.exact_bonus_samples, 1);
        assert_eq!(stats.rollback_count, 0);
        assert_eq!(stats.mtp_cache_reuse_count, 1);
    }

    fn prompt_lookup_cfg() -> PromptLookupConfig {
        PromptLookupConfig {
            min_ngram: 2,
            max_ngram: 3,
            max_draft_tokens: 4,
    ...
    fn prompt_lookup_full_accept_emits_one_pending_token_per_step() {
        ...
        assert_eq!(stats.drafted_tokens, 4);
    ...
    #[test]
    #[serial(mlx_metal)]
    fn prompt_lookup_miss_falls_back_to_one_target_token() {
        let mut scheduler = Scheduler::<ScriptedMtpSchedulerModel>::new(
            1,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        .expect("scheduler startup");
        let id = scheduler.admit(mtp_req(vec![1, 2, 3], 2)).expect("admit");
        let model =
            ScriptedMtpSchedulerModel::new_with_first_token_calls(4, 0, Vec::new(), vec![vec![5]]);
    ...
    #[test]
    #[serial(mlx_metal)]
    fn prompt_lookup_rejected_draft_rolls_back_and_commits_only_target_token() {
        let mut scheduler = Scheduler::<ScriptedMtpSchedulerModel>::new(
            1,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        let events = scheduler
            .step_prompt_lookup(&model)
            .expect("prompt lookup mismatch step");
    ...
    fn prompt_lookup_exact_hybrid_full_accept_commits_live_cache_without_replay() {
        ...
        .expect("scheduler startup");
        let prompt = vec![1, 2, 3, 4, 1, 2, 3];
        let id = scheduler.admit(mtp_req(prompt, 6)).expect("admit");
        let model = ScriptedMtpSchedulerModel::new(4, Vec::new(), vec![vec![1, 2, 3, 4, 5]])
            .with_hybrid_cache()
            .with_required_verify_projection_scope();
        ...
        let events = scheduler
            .step_prompt_lookup(&model)
            .expect("prompt lookup full-accept step");

        assert_eq!(events[0].token, 1);
        assert_eq!(
            model.text_hidden_seq_lens(),
            vec![5],
            "a fully accepted exact window must retain the live mixed-cache state"
        );
        let expected_offset = scheduler
            .get(id)
            .expect("request state")
    ...
    fn prompt_lookup_exact_hybrid_verify_failure_restores_transaction() {
        ...
        .expect("scheduler startup");
        let prompt = vec![1, 2, 3, 4, 1, 2, 3];
        let mut request = mtp_req(prompt, 3);
        request.sampler = Sampler::greedy().with_temperature(0.7).with_seed(29);
        scheduler.admit(request).expect("admit hybrid request");
        let model = ScriptedMtpSchedulerModel::new(4, Vec::new(), vec![vec![9, 0]])
            .with_hybrid_cache()
        ...
        let offsets_before = scheduler
            ...
            .collect::<Vec<_>>();
        let prng_before = scheduler.prng_state.to_vec::<u32>().expect("PRNG before");
        ...
        assert_eq!(
            scheduler.prng_state.to_vec::<u32>().expect("PRNG after"),
            prng_before,
            "failed exact verify must not commit the row PRNG"
        );
    ...
    fn prompt_lookup_hybrid_batch_isolates_rows_with_different_draft_lengths() {
        ...
        .expect("scheduler startup");
        let prompt0 = vec![1, 2, 3, 4, 1, 2, 3];
        let prompt1 = vec![10, 4, 11, 10];
        let id0 = scheduler
            .admit(mtp_req(prompt0.clone(), 6))
            .expect("admit row 0");
        let id1 = scheduler
            .admit(mtp_req(prompt1.clone(), 5))
            .expect("admit row 1");
        ...
        assert_eq!(first[0].token, 4);
        assert_eq!(first[1].token, 4);

        let events = scheduler
            .step_prompt_lookup(&model)
            .expect("prompt lookup batch step");
        assert_eq!(
            events,
            vec![
                StepEvent {
        ...
        let prompt_lookup = scheduler
            ...
            .as_ref()
            .expect("prompt lookup state");
        let row0 = prompt_lookup.rows.get(&0).expect("row 0 lookup state");
    ...
    fn mtp_paged_main_cache_clamps_single_window_draft_budget_to_one() {
        ...
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "mtp-budget-test", 2, 32)
                .expect("prefix config");
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            1,
            32768,
        ...
        s.enable_paged_prefix_cache(config)
            .expect("enable prefix cache");
        s.admit(mtp_req(vec![1, 2], 6)).expect("admit");
        let model = ScriptedMtpSchedulerModel::new(3, vec![4, 5], vec![vec![4, 6]]);
        let cfg = MtpSpeculativeConfig::new(2, Sampler::greedy()).expect("mtp cfg");
    ...
    fn mtp_paged_prefix_clamps_runtime_config_to_one() {
        ...
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "mtp-config-test", 2, 32)
                .expect("prefix config");
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            1,
            32768,
        ...
        s.enable_paged_prefix_cache(config)
            .expect("enable prefix cache");
        s.admit(mtp_req(vec![1, 2], 2)).expect("admit");
        let model = ScriptedMtpSchedulerModel::new(3, vec![4], vec![vec![4, 6]]);
        let cfg = MtpSpeculativeConfig::new(2, Sampler::greedy()).expect("mtp cfg");
    ...
    fn mtp_paged_main_cache_clamps_batched_window_draft_budget_to_one_per_row() {
        ...
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "mtp-batch-budget-test", 2, 64)
                .expect("prefix config");
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            32768,
        ...
        s.enable_paged_prefix_cache(config)
            .expect("enable prefix cache");
        s.admit(mtp_req(vec![1, 2], 6)).expect("admit row 0");
        s.admit(mtp_req(vec![10, 11], 6)).expect("admit row 1");
        ...
        let prefill_cfg = MtpSpeculativeConfig::new(1, Sampler::greedy()).expect("mtp cfg");
        s.prefill_admitted_mtp_batch(&prefill_model, &FakeMtpHead, prefill_cfg)
            .expect("mtp batch prefill");

        let mut mtp_state = s.mtp_state.take().expect("scheduler MTP state");
    ...
    #[test]
    #[serial(mlx_metal)]
    fn mtp_batch_postfill_uses_one_batched_mtp_forward_for_active_rows() {
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        .expect("scheduler startup");
        let id0 = s.admit(mtp_req(vec![1, 2], 5)).expect("admit row 0");
        let id1 = s.admit(mtp_req(vec![10, 11], 5)).expect("admit row 1");
        let model = ScriptedMtpSchedulerModel::new_with_first_token_calls(
            3,
            2,
            vec![4, 6, 8, 9],
            vec![vec![4, 5], vec![6, 7], vec![8, 10], vec![9, 11]],
        );
        let cfg = MtpSpeculativeConfig::new(1, Sampler::greedy()).expect("mtp cfg");
        ...
        *model.project_calls.lock().unwrap() = 0;
        ...
        let step_2 = s
            .step_mtp_batch(&model, &FakeMtpHead)
            .expect("second pending batch step triggers postfill");
        assert_eq!(
            step_2,
            vec![
                StepEvent {
        ...
        assert_eq!(
            ...
            vec![2, 2],
            "postfill should draft both rows in one MTP batch, then commit accepted tails in one MTP batch"
        );
        assert_eq!(
            model.mtp_hidden_seq_lens(),
            vec![1, 1],
        ...
        assert_eq!(
            *model.project_calls.lock().unwrap(),
            1,
            "batched verify should project the whole [B,S,H] hidden tensor once"
        );
        let stats = s.mtp_stats().expect("MTP stats");
        assert_eq!(stats.draft_host_sync_count, 0);
    ...
    fn mtp_long_context_postfill_uses_row_cache_draft_and_batched_verify() {
        ...
        let stats = s.mtp_stats().expect("MTP stats");
        assert_eq!(stats.draft_host_sync_count, 0);
        assert!(stats.verify_accept_host_sync_count > 0);
    }
    ...
    #[test]
    #[serial(mlx_metal)]
    fn mtp_batch_postfill_keeps_mismatch_and_full_accept_rows_independent() {
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        let model = ScriptedMtpSchedulerModel::new_with_first_token_calls(
            ...
            2,
            vec![4, 6, 8, 9],
            vec![vec![4, 5], vec![6, 7], vec![10], vec![9, 11]],
        );
        let cfg = MtpSpeculativeConfig::new(1, Sampler::greedy()).expect("mtp cfg");
        ...
        let step_2 = s
            .step_mtp_batch(&model, &FakeMtpHead)
            .expect("second pending batch step triggers mixed postfill");
        ...
        assert_eq!(
            mtp_state
                ...
                .collect::<Vec<_>>(),
            vec![10],
            "row 0 should keep only the corrected mismatch token"
        );
        assert_eq!(
            mtp_state
                .rows
                .get(&1)
                .expect("row 1 MTP state")
                .pending_tokens
                .iter()
                .copied()
                .collect::<Vec<_>>(),
            vec![9, 11],
            "row 1 should keep accepted draft plus bonus token"
        );
        assert_eq!(mtp_state.stats.rollback_count, 1);
    }
    ...
    fn mtp_batch_subset_postfill_verifies_only_rows_needing_new_window() {
        ...
        let model = ScriptedMtpSchedulerModel::new_with_first_token_calls(
            ...
            2,
            vec![4, 6, 8],
            vec![vec![10], vec![6, 7], vec![8, 12], vec![0]],
        );
        let cfg = MtpSpeculativeConfig::new(1, Sampler::greedy()).expect("mtp cfg");
    ...
    #[test]
    #[serial(mlx_metal)]
    fn mtp_mid_admit_installs_row_state_and_pending_window() {
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        let mut handle = s
            ...
            .expect("begin MTP mid-admit");
        assert!(
            s.admit_mid_chunk_mtp(&mut handle, &model, &FakeMtpHead)
                .expect("MTP mid-admit chunk"),
            "short prompt should complete in one chunk"
        );
        let (id1, first_mid) = s
            .admit_mid_finalize_mtp(handle, &model, &FakeMtpHead)
            .expect("finalize MTP mid-admit");
        assert_eq!(
            first_mid,
            StepEvent {
                id: id1,
        ...
        let mtp_state = s.mtp_state.as_ref().expect("scheduler MTP state");
        assert!(
            mtp_state.rows.contains_key(&0),
            "existing MTP row state must be preserved"
        );
        assert!(
            mtp_state.rows.contains_key(&1),
            "mid-admitted row must install MTP row state"
        );
        assert_eq!(
            mtp_state
                .rows
                ...
                .len(),
            2,
            "mid-admitted row should prefill its first speculative window"
        );

        let step_1 = s
            .step_mtp_batch(&model, &FakeMtpHead)
            .expect("first batch MTP step after mid-admit");
        assert_eq!(
            step_1,
            vec![
                StepEvent {
        ...
        let step_2 = s
            .step_mtp_batch(&model, &FakeMtpHead)
            .expect("second batch MTP step after mid-admit");
        assert_eq!(
            step_2,
            vec![
                StepEvent {
    ...
    #[test]
    #[serial(mlx_metal)]
    fn mtp_batch_step_skips_mid_admit_reserved_rows_until_finalize() {
        let mut s = Scheduler::<ScriptedMtpSchedulerModel>::new(
            2,
            32768,
            crate::core::memory_budget::test_meta_qwen35(),
        ...
        .expect("scheduler startup");
        let id0 = s.admit(mtp_req(vec![1, 2], 4)).expect("admit row 0");
        let model = ScriptedMtpSchedulerModel::new_with_first_token_calls(
            3,
            2,
            vec![4, 6],
        ...
        s.prefill_admitted_mtp_batch(&model, &FakeMtpHead, cfg)
            .expect("mtp batch prefill");
        let id1 = s
            .admit_mid_begin_mtp(mtp_req(vec![10, 11, 12], 3), &model, &FakeMtpHead, cfg)
            .expect("begin MTP mid-admit")
            .request_id;

        let step = s
            .step_mtp_batch(&model, &FakeMtpHead)
            .expect("MTP step should ignore the reserved mid-admit row");
        assert_eq!(
            step,
            vec![StepEvent {
                id: id0,
        ...
        assert!(
            s.get(id1)
                .expect("mid-admit slot remains reserved")
                .generated_tokens
                .is_empty(),
            "reserved mid-admit row should not emit before finalize"
        );
        assert!(
            !s.mtp_state
                .as_ref()
                ...
                .rows
                .contains_key(&1),
            "reserved mid-admit row should not have MTP row state before finalize"
        );
    }

    #[test]
    fn pending_token_fast_path_updates_slot_state() {
        let mut s = TestScheduler::new(2, 32768, crate::core::memory_budget::test_meta_qwen35())
            .expect("scheduler startup");
        let mut req = mk_req(vec![1, 2, 3]);
        req.stop_token_ids = vec![99];
        req.max_new_tokens = 3;
        let id = s.admit(req).expect("admit row");
        let mut pending_tokens = VecDeque::from([42, 43]);

        let event = s
            .emit_pending_token_for_row(0, &mut pending_tokens)
            .expect("emit pending token");
        ...
        assert_eq!(pending_tokens, VecDeque::from([43]));
        let state = s.get(id).expect("row state");
    ...
    #[test]
    fn pending_token_fast_path_marks_length_without_dropping_remaining_queue() {
        let mut s = TestScheduler::new(2, 32768, crate::core::memory_budget::test_meta_qwen35())
            .expect("scheduler startup");
        let mut req = mk_req(vec![1, 2, 3]);
        req.stop_token_ids = vec![99];
    ...
    #[test]
    #[serial(mlx_metal)]
    fn prompt_lookup_state_survives_active_kv_park_and_restore() {
        let model = StepDecodeMaskModel::default();
        let root = std::env::temp_dir().join(format!(
            "ironmlx-prompt-lookup-active-kv-{}",
            uuid::Uuid::new_v4().simple()
        ));
        let config = crate::core::cache::ActiveKvOffloadConfig::enabled(root.clone());
        let stats = crate::core::cache::ActiveKvOffloadSharedStats::new(&config);
        ...
        assert!(parked.prompt_lookup_row.is_some());
        assert_eq!(
            scheduler
                .prompt_lookup_stats()
                .expect("prompt lookup stats")
                .index_entries_current,
            0
        );
    ...
    fn fullpaged_request_ownership_survives_rebuild_and_releases_on_evict() {
        ...
        assert_eq!(
            final_stats.physical.physical_pages_referenced,
            final_stats.physical.immutable_pinned_pages
        );
    ...
    fn fullpaged_request_fork_shares_pool_cows_tail_and_releases_both_branches() {
        ...
        assert_eq!(
            released.physical.physical_pages_referenced,
            released.physical.immutable_pinned_pages
        );
    ...
    #[test]
    #[serial(mlx_metal)]
    fn fullpaged_request_fork_clones_request_local_prompt_lookup_state() {
        let root = std::env::temp_dir().join(format!(
            "ironmlx-prompt-lookup-request-fork-{}",
            uuid::Uuid::new_v4().simple()
        ));
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "prompt-lookup-fork", 3, 64)
                .expect("prefix config");
        let mut scheduler = Scheduler::<StepDecodeMaskModel>::new(
            2,
            32768,
        ...
        scheduler
            .prefill_admitted_prompt_lookup(&model, prompt_lookup_cfg())
            .expect("source prompt lookup prefill");
        let destination = scheduler
            .admit(mk_req(vec![1, 2, 3, 1, 2, 3]))
            .expect("admit destination");
        scheduler
            .register_prompt_lookup_request(destination, prompt_lookup_cfg())
            .expect("register destination prompt lookup state");

        assert!(scheduler
            .fork_request_blocks(source, destination, &model)
            .expect("fork request blocks"));
        let source_row = scheduler.get(source).expect("source state").row_idx;
        let destination_row = scheduler
            .get(destination)
        ...
        let prompt_lookup = scheduler
            ...
            .as_ref()
            .expect("prompt lookup state");
        let source_lookup = prompt_lookup.rows.get(&source_row).expect("source lookup");
        let destination_lookup = prompt_lookup
            .rows
            .get(&destination_row)
        ...
        scheduler.evict(source).expect("evict source");
        assert!(scheduler
            .prompt_lookup_state
            .as_ref()
            .is_some_and(|state| state.rows.contains_key(&destination_row)));
        scheduler.evict(destination).expect("evict destination");
        scheduler.evict_all().expect("reset scheduler");
    ...
    #[test]
    #[serial(mlx_metal)]
    fn fullpaged_immutable_prefix_reuses_idle_blocks_across_admission() {
        let root = std::env::temp_dir().join(format!(
            "ironmlx-immutable-idle-prefix-{}",
            uuid::Uuid::new_v4().simple()
        ));
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "immutable-idle", 2, 64)
                .expect("prefix config");
        let mut scheduler = Scheduler::<StepDecodeMaskModel>::new(
            1,
            32768,
        ...
        scheduler
            ...
            .expect("warm prefill");
        let warm_stats = scheduler.request_owned_kv_stats();
        assert_eq!(warm_stats.immutable_prefix.blocks, 2);
        assert_eq!(warm_stats.immutable_prefix.published_blocks, 2);
        assert_eq!(warm_stats.physical.immutable_pinned_tables, 2);
        scheduler.evict(warm).expect("evict warm request");
        scheduler.evict_all().expect("reset scheduler phase");

        let hit = scheduler
            .admit(mk_req(vec![1, 2, 3, 4, 9, 10]))
            .expect("admit hit request");
        let hit_model = StepDecodeMaskModel::default();
        let events = scheduler
            .prefill_admitted(&hit_model)
            .expect("immutable hit prefill");
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].id, hit);
        assert_eq!(hit_model.hidden_seq_lens(), vec![1]);
        assert_eq!(hit_model.forward_seq_lens(), vec![1]);
        let hit_stats = scheduler.request_owned_kv_stats();
        assert_eq!(hit_stats.immutable_prefix.restored_blocks, 2);
        assert_eq!(hit_stats.immutable_prefix.idle_block_hits, 2);
        assert_eq!(hit_stats.immutable_prefix.active_block_hits, 0);
        assert!(hit_stats.immutable_prefix.dedup_saved_bytes > 0);
        ...
        scheduler.evict(hit).expect("evict hit request");
        scheduler.evict_all().expect("reset after hit");
        let retained = scheduler.request_owned_kv_stats();
        assert_eq!(retained.request_tables, 0);
        assert_eq!(retained.physical.request_owned_tables, 0);
        assert_eq!(retained.physical.physical_pages_referenced, 2);
        assert_eq!(retained.physical.immutable_pinned_pages, 2);
        scheduler.discard_retained_immutable_prefix_cache();
        crate::core::cache::process_async_prefix_store_queue().wait_idle();
        std::fs::remove_dir_all(root).expect("cleanup prefix cache");
    }
    ...
    fn fullpaged_immutable_prefix_reuses_active_blocks_for_mid_admission() {
        ...
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "immutable-active", 2, 64)
                .expect("prefix config");
        let mut scheduler = Scheduler::<StepDecodeMaskModel>::new(
            2,
            32768,
        ...
        let mut handle = scheduler
            .admit_mid_begin(mk_req(vec![1, 2, 3, 4, 9]), &model)
            .expect("begin active immutable hit");
        assert!(handle.immutable_in_place);
        assert_eq!(handle.chunk_start, 4);
        assert!(scheduler
            .admit_mid_chunk(&mut handle, &model)
            .expect("prefill private suffix"));
        let (destination, event) = scheduler
            .admit_mid_finalize(handle, &model)
            .expect("finalize active immutable hit");
        assert_eq!(event.id, destination);

        let after = scheduler.request_owned_kv_stats();
        assert_eq!(after.immutable_prefix.active_block_hits, 2);
        assert_eq!(after.immutable_prefix.idle_block_hits, 0);
        assert_eq!(
            after.physical.adopt_page_copies,
            before.physical.adopt_page_copies
        );
        assert_eq!(
            after.physical.cow_page_copies,
            before.physical.cow_page_copies
        );
        assert_eq!(after.layout_rebuild_page_copies, 0);
        assert_eq!(after.physical.orphan_pages, 0);
        scheduler
            .validate_request_owned_kv_invariants()
            .expect("active immutable hit invariants");

        scheduler.evict(source).expect("evict source");
        scheduler.evict(destination).expect("evict destination");
        scheduler.evict_all().expect("reset after active hit");
        scheduler.discard_retained_immutable_prefix_cache();
        crate::core::cache::process_async_prefix_store_queue().wait_idle();
        std::fs::remove_dir_all(root).expect("cleanup prefix cache");
    }
    ...
    #[test]
    #[serial(mlx_metal)]
    fn fullpaged_immutable_prefix_uses_exact_chain_and_reclaims_idle_leaf() {
        let root = std::env::temp_dir().join(format!(
            "ironmlx-immutable-chain-prefix-{}",
            uuid::Uuid::new_v4().simple()
        ));
        let config =
            crate::core::cache::PagedPrefixCacheConfig::new(&root, "immutable-chain", 2, 3)
                .expect("prefix config");
        let mut scheduler = Scheduler::<StepDecodeMaskModel>::new(
            1,
            32768,
        ...
        let warm = scheduler
            .admit(mk_req(vec![1, 2, 3, 4, 5]))
            .expect("admit warm chain");
        scheduler
            .prefill_admitted(&StepDecodeMaskModel::default())
            .expect("warm chain prefill");
        scheduler.evict(warm).expect("evict warm chain");
        scheduler.evict_all().expect("reset scheduler phase");

        let branch = scheduler
            .admit(mk_req(vec![1, 2, 7, 8, 9]))
            .expect("admit branch");
        let branch_model = StepDecodeMaskModel::default();
        scheduler
            .prefill_admitted(&branch_model)
            .expect("branch prefill");
        assert_eq!(branch_model.hidden_seq_lens(), vec![2]);
        assert_eq!(branch_model.forward_seq_lens(), vec![1]);
        let stats = scheduler.request_owned_kv_stats();
        assert_eq!(stats.immutable_prefix.restored_blocks, 1);
        assert_eq!(stats.immutable_prefix.idle_block_hits, 1);
        assert!(stats.immutable_prefix.lookup_misses >= 1);
        assert!(stats.immutable_prefix.evicted_blocks >= 1);
        assert_eq!(stats.immutable_prefix.blocks, 2);
        assert_eq!(stats.physical.cow_page_copies, 0);
        assert_eq!(stats.physical.orphan_pages, 0);
        scheduler
            .validate_request_owned_kv_invariants()
            .expect("branch chain invariants");

        scheduler.evict(branch).expect("evict branch");
        scheduler.evict_all().expect("reset after branch");
        scheduler.discard_retained_immutable_prefix_cache();
        crate::core::cache::process_async_prefix_store_queue().wait_idle();
        std::fs::remove_dir_all(root).expect("cleanup prefix cache");
    }
    ...
    #[test]
    fn scheduler_gc_finished_rows_removes_gemma4_drafter_row_state_for_evicted_slots() {
        use std::collections::HashMap;
        use tokio::sync::mpsc;
        ...
        s.gemma4_drafter_state = Some(SchedulerGemma4DrafterState {
            cfg: MtpSpeculativeConfig::new(2, Sampler::greedy()).expect("mtp config"),
            rows: HashMap::from([
                (
                    0,
                    SchedulerGemma4DrafterRowState {
        ...
        let drafter_state = s
            .gemma4_drafter_state
            .as_ref()
            .expect("remaining active row keeps drafter state");
        assert!(
            !drafter_state.rows.contains_key(&0),
            "evicted row 0 drafter state must be dropped"
        );
        assert!(
            drafter_state.rows.contains_key(&1),
            "active row 1 drafter state must remain"
        );
    }
    ...
    fn scheduler_evict_removes_gemma4_drafter_row_state_for_evicted_slot() {
        ...
        .expect("scheduler startup");
        let id_a = s.admit(mk_req(vec![1, 2, 3])).expect("admit a");
        let _id_b = s.admit(mk_req(vec![4, 5, 6])).expect("admit b");
        s.force_phase(Phase::Decoding);
        ...
        s.evict(id_a).expect("evict id_a");
