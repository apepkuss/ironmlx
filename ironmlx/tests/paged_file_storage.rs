use ironmlx::core::cache::PagedKVCache;
use ironmlx::core::model_input::{build_batch_attention_mask, build_per_row_decode_mask};
use mlx::{Array, Dtype};
use std::{
    fs,
    path::PathBuf,
...
fn paged_kv_hot_cold_allows_direct_prefix_restore_when_empty_and_budget_fits() {
    ...
    empty
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(root.join("empty"), 4, 1)
    ...
    live.enable_hot_cold_tiering(
        ironmlx::core::page_storage::file_paged_kv_config(root.join("live"), 4, 1)
            .expect("hot/cold config"),
    )
    .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_direct_prefix_restore_installs_cold_pages_without_swap_in() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 1, 1)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_pressure_shrink_demotes_resident_pages_idempotently() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 4, 1)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_streaming_decode_loads_offloaded_segments_once() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 1, 4)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_decode_stages_context_to_paged_kernel_when_budget_allows() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 1, 8)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_prefill_keeps_resident_when_budget_allows() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 1, 7)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_prefill_streaming_reads_cold_pages_once_per_row() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 2, 4)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
...
fn paged_kv_hot_cold_repeated_decode_matches_dense_sdpa() {
    ...
    paged
        .enable_hot_cold_tiering(
            ironmlx::core::page_storage::file_paged_kv_config(&root, 2, 2)
                .expect("hot/cold config"),
        )
        .expect("enable hot/cold tiering");
