...
fn prepare_fixture_images(
    ...
    for path in paths {
        ...
        let (pv, gh, gw) =
            ironmlx_lm::models::qwen3_5::image_processor::preprocess(&bytes).expect("preprocess");
...
fn generation_request(
    tokenizer: &ironmlx_lm::core::Tokenizer,
    prompt_ids: Vec<u32>,
    ...
    ironmlx_runtime::core::generation_types::GenerateRequest {
        ...
        image_token_id: tokenizer
            ...
            .map(|id| id as i32)
            .unwrap_or(ironmlx_lm::core::model_input::IMAGE_TOKEN_ID),
        constraint: None,
    }
}
#[test]
...
fn qwen36_moe_text_generation_smoke_real_checkpoint() {
    ...
    let loader = ironmlx_lm::core::Loader::open(&dir).expect("open");
    let tokenizer = ironmlx_lm::core::Tokenizer::from_loader(&loader).expect("tokenizer");
    let model = Qwen36MoeModel::from_loader(&loader).expect("model");
    let prompt = tokenizer
        .apply_chat_template(
            &[ironmlx_lm::core::Message {
                role: "user".to_owned(),
    ...
    let mut stream = ironmlx_runtime::core::generate::GenerationStream::new_text_only(
...
fn qwen36_moe_single_image_generation_smoke_real_checkpoint() {
    ...
    let loader = ironmlx_lm::core::Loader::open_multimodal(&dir).expect("open_multimodal");
    let tokenizer = ironmlx_lm::core::Tokenizer::from_loader(&loader).expect("tokenizer");
    let model = Qwen36MoeModel::from_loader(&loader).expect("model");
    ...
    let mut stream =
        ironmlx_runtime::core::generate::GenerationStream::new(&model, &tokenizer, request)
            .expect("stream");
    assert!(stream.next_token().expect("next token").is_some());
}
...
fn qwen36_moe_multi_image_generation_smoke_real_checkpoint() {
    ...
    let (pixel_values, grids, mut content) = prepare_fixture_images(
        &[
            "../ironmlx/tests/fixtures/qwen35_vl/multi_image/image_0.jpg",
            "../ironmlx/tests/fixtures/qwen35_vl/multi_image/image_1.jpg",
        ],
        merge,
    );
