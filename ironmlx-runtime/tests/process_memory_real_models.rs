...
use ironmlx_runtime::core::process_memory::global_process_memory_governor;

use ironmlx_runtime::core::process_memory::native_memory_telemetry;

use ironmlx_runtime::core::process_memory::ColdMaterializationTracker;

use ironmlx_runtime::core::process_memory::MaterializationComponents;
...
fn fixture_image() -> Vec<u8> {
    std::fs::read(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../ironmlx/tests/fixtures/qwen35_vl/coco_sample.jpg"),
    )
...
fn calibrate<M: DenseVlMethods>(
    ...
    let estimated_bytes = model
        ...
        .and_then(ironmlx_runtime::core::memory_budget::vision_prefill_reservation_bytes)
...
fn qwen35_real_vision_peak_is_covered() {
    ...
    let model =
        ironmlx_lm::models::qwen3_5::Qwen35Model::from_loader(&loader).expect("load Qwen3.5 model");
    let (pixels, grid_h, grid_w) =
        ironmlx_lm::models::qwen3_5::image_processor::preprocess(&fixture_image())
...
fn gemma4_real_vision_peak_is_covered() {
    ...
    let config = ironmlx_lm::models::gemma4::Gemma4Config::from_loader(&loader)
        .expect("parse Gemma4 config");
    let vision_config = config.vision_config.as_ref().expect("Gemma4 vision config");
    let processed =
        ironmlx_lm::models::gemma4::image_processor::preprocess(&fixture_image(), vision_config)
            .expect("preprocess Gemma4 image");
    let model = ironmlx_lm::models::gemma4::Gemma4Model::from_loader_with_config(&loader, config)
...
fn minicpmv46_real_vision_peak_is_covered() {
    ...
    let model = ironmlx_lm::models::minicpmv4_6::MiniCpmV46Model::from_loader(&loader)
        .expect("load MiniCPM-V-4.6 model");
    let (pixels, grid_h, grid_w) =
        ironmlx_lm::models::minicpmv4_6::image_processor::preprocess(&fixture_image())
