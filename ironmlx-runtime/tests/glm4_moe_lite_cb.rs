...
//!     cargo test -p ironmlx-runtime --release --test glm4_moe_lite_cb -- --ignored --nocapture --test-threads=1
...
use ironmlx_lm::models::glm4_moe_lite::Glm4MoeLiteModel;
...
use ironmlx_runtime::core::scheduler_actor::SchedulerActorHandle;
...
fn tokenize_prompt(tokenizer: &Tokenizer, text: &str) -> Vec<u32> {
    let msgs = vec![ironmlx_lm::core::Message {
        role: "user".into(),
        content: text.into(),
    }];
...
async fn submit_admit(
    ...
    req: GenerateRequest,
) -> ironmlx_runtime::Result<AdmitReply> {
