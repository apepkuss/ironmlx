...
mod tests {
    ...
    fn position_ids_vl_batched_b2_each_one_image_matches_per_stream() {
        ...
        let per_row_grid_thw = vec![Some(&row0_grids[..]), Some(&row1_grids[..])];
        ...
        for s in 0..3 {
            for col in 0..len0 {
                let bat = batched_flat[s * 2 * b_len + col];
            ...
            for col in 0..len1 {
                let bat = batched_flat[s * 2 * b_len + b_len + col];
    ...
    fn position_ids_vl_batched_mixed_text_vl_matches_per_stream() {
        ...
        let per_row_prompt_ids: Vec<&[i32]> = vec![&row0_ids[..], &row1_ids[..]];
        let per_row_grid_thw = vec![None, Some(&row1_grids[..])];
        let batched = build_position_ids_vl_batched(
            &per_row_prompt_ids,
            &per_row_grid_thw,
            image_token_id,
...
mod per_row_decode_mask_tests {
    ...
    fn mask_per_row_decode_ragged() {
        ...
        // Row 1:
        for (k, value) in v[5..10].iter().enumerate() {
            assert_eq!(*value, 0.0, "row 1 position {} should be 0", k);
        }
    }
