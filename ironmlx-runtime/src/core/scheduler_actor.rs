...
//! the first admit starts the configured admission deadline timer; further admits
...
//! route through [`Scheduler::admit_mid_begin`] (B=1 temp-cache prefill +
...
/// Commands accepted by the actor. 3b-2 ships only [`Admit`](SchedulerCommand::Admit); later
...
pub mod test_support {
    ...
    pub const SchedulerActorFakeModel: SchedulerActorFakeModel = SchedulerActorFakeModel {
        forward_delay: Duration::ZERO,
        mtp_accepted_prefix_restore: false,
    };

    impl SchedulerActorFakeModel {
        pub fn with_forward_delay(forward_delay: Duration) -> Self {
            Self {
                forward_delay,
                mtp_accepted_prefix_restore: false,
            }
        ...
        #[cfg(test)]
        pub(super) fn with_mtp_accepted_prefix_restore(forward_delay: Duration) -> Self {
            Self {
                forward_delay,
    ...
    #[derive(Clone, Copy)]
    pub struct SchedulerActorFakeMtpHead;
    ...
    pub async fn wait_for_scheduler_resources_to_be_released(handle: &SchedulerActorHandle) {
    ...
    pub fn test_scheduler_handle() -> SchedulerActorHandle {
        ...
        SchedulerActorHandle {
            ...
            immutable_prefix_blocks: ImmutablePrefixBlockSharedStats::new(false),
        }
    }
}
