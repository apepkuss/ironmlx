...
| `models/(vision\|qwen3_5/(cross_modal\|image_processor)\|qwen3_5_moe)` | `p6_qwen35_vl_logits_match` | vl |
...
MoE-VL end-to-end coverage is intentionally outside the sweep scripts because it
loads the full local checkpoint and runs HTTP + semantic checks. Run
`scripts/p6_moe_vl_smoke.sh` separately for MoE-VL changes.
