...
pub enum AttnKind {
    ...
    Full,
    /// Gated delta-net linear attention SSM (P3b3). Consumes [`crate::core::cache::GatedDeltaCache`].
    Linear,
}
...
///
/// `pub` (not `pub(crate)`) so integration tests in `ironmlx-lm/tests/` can construct it
...
pub use crate::core::cache::layer::{
...
mod tests {
    use super::*;
    use crate::core::cache::prefix_payload::PrefixLayerPayload;
    ...
    fn build_inputs_fp32(cfg: DecoderLayerConfig) -> (Array, Mrope, Array, Array) {
        ...
        let position_ids = mlx::ops::shape::broadcast_to_on(&pos1d, [n_streams, b, s], ()).unwrap();
