...
/// Enter model execution routing; drop this guard on the same thread.
#[doc(hidden)]
pub fn position_stable_linear_scope() -> impl Drop {
    position_stable_linear::scope()
}

/// Enter model execution routing; drop this guard on the same thread.
#[doc(hidden)]
pub fn batch_stable_qmm_context_scope() -> impl Drop {
    batch_stable_qmm::context_scope()
}

/// Enter model execution routing; drop this guard on the same thread.
#[doc(hidden)]
pub fn gemma4_verify_attention_scope() -> impl Drop {
    gemma4_verify_attention::scope()
}

/// Enter model execution routing; drop this guard on the same thread.
#[doc(hidden)]
pub fn exact_affine8_b4_q2_scope() -> impl Drop {
    position_stable_qmm::exact_affine8_b4_q2_scope()
}

/// Whether the current backend supports the exact affine8 verify route.
#[doc(hidden)]
