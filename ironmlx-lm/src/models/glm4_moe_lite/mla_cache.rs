...
mod tests {
    ...
    fn adopt_row_copies_src_row_offset_and_data() {
        ...
        let c_kv: Array = {
            let mut d = [0.0_f32; 2 * 2 * 4]; // [B=2,1,S=2,4]
            for v in d.iter_mut().skip(2 * 4) {
                *v = 5.0;
            } // row1 block
            (&d[..], (2_i32, 1, 2, 4)).try_into().unwrap()
        };
        let k_pe: Array = {
            let mut d = [0.0_f32; 2 * 2 * 2];
            for v in d.iter_mut().skip(2 * 2) {
                *v = 6.0;
            }
