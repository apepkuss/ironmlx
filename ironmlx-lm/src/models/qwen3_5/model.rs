...
impl Qwen35Model {
    ...
    pub fn from_loader_dflash2(loader: &mut Loader) -> Result<Self> {
    ...
    /// [`crate::models::qwen3_5::cross_modal::replace_image_tokens`] (or its chunked equivalent).
    ///
    ...
    /// [`crate::models::qwen3_5::cross_modal::replace_image_tokens`]. Per-row concat ordering must
...
mod tests {
    ...
    /// Run with:
    /// ```
    /// QWEN35_MODEL=<path> cargo test -p ironmlx-lm --lib --release forward_vl_text_only_matches_forward_on -- --ignored
    /// ```
    #[test]
    ...
    /// Run with:
    /// ```
    /// IRONMLX_MODEL_DIR=<path> cargo test -p ironmlx-lm --lib \
    ...
    fn batched_prefill_vl_text_only_matches_batched_prefill() {
        ...
        let per_row_grids: Vec<Option<&[ImageGrid]>> = vec![None, None];
        let logits_vl = model
            .batched_prefill_vl(
                &input_ids,
                &position_ids,
