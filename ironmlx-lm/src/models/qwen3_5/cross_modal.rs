...
mod tests {
    ...
    fn replaces_image_placeholders_b2_row1_multi_image() {
        ...
        for k in 0..4 {
            assert_eq!(v[4 + k], 5.0, "row 0 col 1 hidden[{k}]");
        }
        // Row 1 col 0 (IMG #1) = 11.0
        for k in 0..4 {
            assert_eq!(v[3 * 4 + k], 11.0, "row 1 col 0 hidden[{k}]");
        }
        // Row 1 col 2 (IMG #2) = 17.0
        for k in 0..4 {
            assert_eq!(v[(3 + 2) * 4 + k], 17.0, "row 1 col 2 hidden[{k}]");
        }
        // Row 0 col 0/2 (text) = 1.0
        for col in [0, 2] {
            for k in 0..4 {
                assert_eq!(v[col * 4 + k], 1.0);
            }
        }
        // Row 1 col 1 (text) = 1.0
        for k in 0..4 {
            assert_eq!(v[(3 + 1) * 4 + k], 1.0);
        }
    }
}
