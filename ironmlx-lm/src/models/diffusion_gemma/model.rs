...
impl DiffusionGemmaCache {
    /// Whether the cache contains no encoded token positions.
...
mod tests {
    ...
    fn encoder_mask_allows_bidirectional_attention_inside_visual_blocks() {
        ...
        assert_eq!(values[4 + 2], 0.0, "image token attends future image token");
        assert_eq!(
            values[2 * 4 + 1],
            0.0,
        ...
        assert!(
            values[4 + 3].is_infinite() && values[4 + 3].is_sign_negative(),
