...
pub fn gemma4_shared_kv_from_cache_on(
...
pub fn draft_position_for_shared_kv(kv_valid_len: i32) -> i32 {
    (kv_valid_len - 1).max(0)
}

/// Extract one committed row from a full batched target-cache view.
pub fn shared_kv_row_prefix_on(
    states: &Gemma4SharedKvStates,
    row_idx: usize,
    prefix_len: i32,
    target: impl Into<StreamOrDevice>,
...
pub fn shared_kv_row_trim_suffix_on(
    states: &Gemma4SharedKvStates,
    row_idx: usize,
    trim: usize,
    target: impl Into<StreamOrDevice>,
