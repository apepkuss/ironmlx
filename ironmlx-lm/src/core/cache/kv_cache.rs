...
mod tests {
    ...
    fn kvcache_turboquant_adopt_row_from_preserves_existing_packed_rows_when_growing() {
        ...
        let short_k_data: Vec<f32> = (0..(2 * 4 * 8))
            .map(|i| 0.5 + ((i as f32) * 0.071).sin())
            .collect();
        let short_v_data: Vec<f32> = (0..(2 * 4 * 8))
        ...
        let long_k_data: Vec<f32> = (0..(2 * 8 * 8))
            .map(|i| 1.0 + ((i as f32) * 0.097).sin())
            .collect();
        let long_v_data: Vec<f32> = (0..(2 * 8 * 8))
    ...
    fn kvcache_multi_step_accumulation() {
        ...
        k1_data.extend(std::iter::repeat_n(1.0_f32, n_per_row_step1));
        k1_data.extend(std::iter::repeat_n(2.0_f32, n_per_row_step1));
        ...
        k2_data.extend(std::iter::repeat_n(3.0_f32, n_per_row_step1));
        k2_data.extend(std::iter::repeat_n(4.0_f32, n_per_row_step1));
    ...
    fn kvcache_per_row_data_isolation() {
        ...
        k_data.extend(std::iter::repeat_n(1.0_f32, n_per_row));
        k_data.extend(std::iter::repeat_n(2.0_f32, n_per_row));
        let v_data: Vec<f32> = k_data.iter().map(|x| x * 10.0).collect();
        ...
        for (offset, &value) in kf_vec[0..row_stride].iter().enumerate() {
            let i = offset;
            assert_eq!(value, 1.0_f32, "K row 0 slab corrupted at index {i}");
        }
        // K row 1: every element in slab [1 * row_stride .. 2 * row_stride] must be 2.0
        for (offset, &value) in kf_vec[row_stride..(2 * row_stride)].iter().enumerate() {
            let i = row_stride + offset;
            assert_eq!(value, 2.0_f32, "K row 1 slab corrupted at index {i}");
        }
        ...
        for (offset, &value) in vf_vec[0..row_stride].iter().enumerate() {
            let i = offset;
            assert_eq!(value, 10.0_f32, "V row 0 slab corrupted at index {i}");
        }
        for (offset, &value) in vf_vec[row_stride..(2 * row_stride)].iter().enumerate() {
            let i = row_stride + offset;
            assert_eq!(value, 20.0_f32, "V row 1 slab corrupted at index {i}");
        }
    }
    ...
    fn kvcache_adopt_row_from_basic() {
        ...
        let k_data: Vec<f32> = std::iter::repeat_n(7.0_f32, n_per_row).collect();
        let v_data: Vec<f32> = std::iter::repeat_n(70.0_f32, n_per_row).collect();
        ...
        let probe_k_data: Vec<f32> = vec![0.0_f32; ((2 * 4) * 256) as usize];
        let probe_v_data: Vec<f32> = vec![0.0_f32; ((2 * 4) * 256) as usize];
        ...
        assert_eq!(
            kf_vec[stride_batch], 7.0_f32,
            "dst row 1 col 0 head 0 dim 0 should be 7.0 (adopted K)"
        );
        assert_eq!(
            vf_vec[stride_batch], 70.0_f32,
    ...
    fn kvcache_adopt_row_from_out_of_bounds_err() {
        ...
        let k_data: Vec<f32> = std::iter::repeat_n(1.0_f32, n_per_row).collect();
        let v_data: Vec<f32> = std::iter::repeat_n(1.0_f32, n_per_row).collect();
