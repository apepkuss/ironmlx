...
mod tests {
    ...
    fn gdcache_adopt_row_from_state_and_offset() {
        ...
        for (offset, &value) in conv_vec[0..conv_stride_row].iter().enumerate() {
            let i = offset;
            assert_eq!(value, 0.0_f32, "dst.conv_state row 0 corrupted at {i}");
        }
        for (offset, &value) in conv_vec[conv_stride_row..(2 * conv_stride_row)]
            .iter()
            .enumerate()
        {
            let i = conv_stride_row + offset;
            assert_eq!(value, 1.0_f32, "dst.conv_state row 1 wrong at {i}");
        }
        ...
        for (offset, &value) in rec_vec[0..rec_stride_row].iter().enumerate() {
            let i = offset;
            assert_eq!(value, 0.0_f32, "dst.rec row 0 corrupted at {i}");
        }
        for (offset, &value) in rec_vec[rec_stride_row..(2 * rec_stride_row)]
            .iter()
            .enumerate()
        {
            let i = rec_stride_row + offset;
            assert_eq!(value, 2.0_f32, "dst.rec row 1 wrong at {i}");
        }
    }
    ...
    fn gdcache_prefix_state_round_trips_single_row() {
        ...
        let mut conv_data = [0.0_f32; 2 * 3 * 8];
