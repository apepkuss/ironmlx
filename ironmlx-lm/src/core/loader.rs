...
pub struct LoadedTensorComponentBytes {
    pub text: usize,
    pub vision: usize,
}
...
impl WeightSource for Loader {
    fn tensor(&self, key: &str) -> Result<&Array> {
        Loader::tensor(self, key)
    }

    fn tensor_opt(&self, key: &str) -> Option<&Array> {
        Loader::tensor_opt(self, key)
    }

    fn contains(&self, key: &str) -> bool {
        Loader::contains(self, key)
    }

    fn quant_meta_for(&self, prefix: &str) -> Option<QuantMeta> {
        Loader::quant_meta_for(self, prefix)
    }
}
...
impl Loader {
    ...
    pub fn loaded_tensor_component_bytes(&self) -> LoadedTensorComponentBytes {
...
mod tests {
    ...
    fn quant_meta_for_prefers_override_then_falls_back_to_global() {
        ...
        let source: &dyn WeightSource = &loader;
        assert_eq!(
            source.quant_meta_for("model.layers.0.mlp.gate"),
            Some(override_meta)
        );
        assert_eq!(
            source.quant_meta_for("model.layers.0.self_attn.q_proj"),
            Some(global)
        );
        assert_eq!(
            source.tensor("absent").unwrap_err().to_string(),
            "Loader: missing tensor key `absent`"
        );
        assert!(!source.contains("absent"));
        assert!(source.tensor_opt("absent").is_none());
    }
