...
//!
//! Loads .npy fixtures from `tests/fixtures/mrope/` (generated by
//! `gen_fixture.py` against an independent Python MRoPE reference) and
//! verifies that:
//!   - `Mrope::cos_sin` matches the reference cos/sin tables
//!   - `Mrope::apply` matches the reference rotated Q/K
//!   - The full rotary + SDPA pipeline (Mrope::apply → fast::SDPA) matches
//!     the reference attention output
//!
//! Tolerances:
//!   - cos/sin (fp32):    atol = 1e-5
//!   - apply (bf16):      atol = 1e-3 (limited by bf16 rounding)
//!   - rotary+SDPA (bf16): atol = 1e-3 (same accumulator path as apply)

use mlx::{Array, Dtype};

use ironmlx_lm::nn::Mrope;

const FIXTURE_DIR: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../ironmlx/tests/fixtures/mrope"
);

/// Head dim pinned by the fixture data (Qwen3.5-4B). All tests use this
/// value to compute the SDPA scale; must match `HEAD_DIM` in `gen_fixture.py`.
const HEAD_DIM: f32 = 256.0;
