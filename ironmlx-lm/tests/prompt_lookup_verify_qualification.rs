...
use ironmlx_lm::core::model_input::{build_batched_append_attention_mask, build_position_ids};
use ironmlx_lm::core::{Loader, Model, Tokenizer};
use ironmlx_lm::models::{
...
fn qualify_case<M: Model>(
    ...
    let verify_qmm = ironmlx_lm::nn::verify_qmm_scope();
    let hidden = model.forward_text_hidden(
        &verify_input,
        &positions,
        Some(&per_row_lens),
    ...
    for (row, tokens) in isolated_projection_tokens.iter_mut().enumerate() {
        for depth in 0..verify_width {
            ...
            tokens.push(argmax_tokens(&position_logits)?[0]);
        }
    }
...
fn qualify_identical_row_decode<M: Model>(
    ...
    let positions =
        ironmlx_lm::core::model_input::build_position_ids_batched(&per_row_lens, prefix_len_i32)?;
