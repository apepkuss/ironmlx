...
//! cargo test -p ironmlx-lm --test qwen35_vl_logits_match --release -- --ignored
//! ```
//!
//! Before running, generate fixtures (one time):
//! ```text
//! QWEN35_MODEL=/path/to/Qwen3.5-4B-MLX-4bit \
//! ~/.venvs/mlxvlm-ref/bin/python ironmlx/tests/fixtures/qwen35_vl/gen_fixture.py
//! ```

use mlx::{ops, Array, Dtype};

use ironmlx_lm::core::{
    model_input::{build_position_ids_vl, IMAGE_TOKEN_ID},
    Loader,
};
...
const FIXTURE_DIR: &str = concat!(
    ...
    "/../ironmlx/tests/fixtures/qwen35_vl"
);
...
fn p6_qwen35_vl_logits_match() {
    ...
    let logits = model
        .forward_vl(
            ...
            Some(&grids_tup),
            ironmlx_lm::core::model_input::IMAGE_TOKEN_ID,
            (),
        )
        .expect("forward_vl");
