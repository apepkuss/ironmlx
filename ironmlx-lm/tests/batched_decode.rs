...
//!      `batched_prefill.rs` for the rationale).
//!
//! Run with:
//!   QWEN35_MODEL=/path/to/model \
//!   MLX_DIR=$HOME/.local/mlx \
//!   cargo test -p ironmlx-lm --release --test batched_decode -- --ignored --nocapture
...
fn per_stream_reference(model: &Qwen35Model, prompt: &[u32], n_decode: usize) -> Vec<Array> {
    ...
    // Prefill.
    let input_ids: Array = (prompt, &[1_i32, s][..]).try_into().expect("input_ids");
...
fn run_point(model: &Qwen35Model, prompt_lens: &[i32], seed_base: u64, stats: &mut MatrixStats) {
    ...
    for p in &prompts {
        ...
        packed.extend(std::iter::repeat_n(PAD_TOKEN_ID, pad_n));
    }
    ...
    for (i, reference) in refs.iter().enumerate() {
        ...
        let d = max_abs_diff_f32(&row_flat, &reference[0]);
        let our_arg = argmax(&row_flat);
        let ref_arg = argmax(&reference[0]);
    ...
    for k in 1..=DECODE_STEPS {
        ...
        for (i, reference) in refs.iter().enumerate() {
            ...
            let d = max_abs_diff_f32(&row_flat, &reference[k]);
            let our_arg = argmax(&row_flat);
            let ref_arg = argmax(&reference[k]);
            let argmax_match = our_arg == ref_arg;
            eprintln!(
