...
impl Sampler {
    ...
    pub fn requires_sampling_history(&self) -> bool {
    ...
    fn distribution(&self, logits: &Array, history: &[u32]) -> Result<SamplingDistribution> {
        anyhow::ensure!(
            self.temperature > 0.0,
...
pub struct PreparedTargetTokenSampling {
    probabilities: Array,
...
impl PreparedTargetTokenSampling {
    pub fn probabilities(&self) -> &Array {
        &self.probabilities
    }

    pub fn compact_probabilities(&self) -> Option<&Array> {
        self.compact_probabilities.as_ref()
    }

    pub fn compact_indices(&self) -> Option<&Array> {
        self.compact_indices.as_ref()
    }

    pub fn sample(self, uniforms: &[f32]) -> Result<Vec<u32>> {
        anyhow::ensure!(
...
pub fn prepare_target_tokens_with_uniforms_batch(
    samplers: &[&Sampler],
    logits: &Array,
    histories: &[&[u32]],
...
pub fn draw_uniforms(prng_state_row: &mut Array, count: usize) -> Result<Vec<f32>> {
    Ok(prepare_uniforms(prng_state_row, count)?.to_vec()?)
}

pub fn prepare_uniforms(prng_state_row: &mut Array, count: usize) -> Result<Array> {
    anyhow::ensure!(
...
fn apply_top_p_batched(probs: &Array, top_p_per_row: &Array) -> Result<Array> {
    ...
    let zeros: Array = (&vec![0_f32; probs.size()][..], probs.shape().as_slice()).try_into()?;
...
/// Explicit test access to the same configured distribution pipeline used in production.
/// This module is not enabled by default and carries no model verification policy.
#[cfg(feature = "test-support")]
pub mod test_support {
    use super::*;
    #[derive(Debug, Clone)]
    pub struct Distribution(SamplingDistribution);
    impl Distribution {
        pub fn new(probabilities: Vec<f32>) -> Result<Self> {
            SamplingDistribution::new(probabilities).map(Self)
        }
        ...
        pub fn sample_with_uniform(&self, uniform: f32) -> Result<u32> {
    ...
    pub fn distributions(
        ...
        sampler
            .distributions(logits, histories)
            .map(|rows| rows.into_iter().map(Distribution).collect())
    }
}
