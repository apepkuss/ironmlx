...
use crate::weights::{logical_width_from_packed, QuantMode, WeightSource};
use crate::Result;
...
impl Linear {
    ...
    /// model layers's concatenated input
    ...
    pub fn quantized_parts(&self) -> Option<QuantizedLinearParts<'_>> {
        match &self.inner {
            LinearImpl::Fp { .. } => None,
            LinearImpl::Quant {
                weight,
...
mod tests {
    ...
    fn new_quant_round_trips_via_from_loader_shape() {
        ...
        let scales_data = vec![0.01_f32; out as usize]; // in/group_size=1
