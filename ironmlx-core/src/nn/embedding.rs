...
use crate::weights::{QuantMode, WeightSource};
use crate::Result;
use anyhow::anyhow;
...
/// Borrowed quantized embedding parameters for model-specific output projection.
#[derive(Clone, Copy)]
pub struct QuantizedEmbeddingParts<'a> {
    pub weight: &'a Array,
    pub scales: &'a Array,
...
impl Embedding {
    pub fn quantized_parts(&self) -> Option<QuantizedEmbeddingParts<'_>> {
        match &self.inner {
            EmbeddingImpl::Fp { .. } => None,
            EmbeddingImpl::Quant {
                weight,
                scales,
                biases,
    ...
    /// `#[doc(hidden)]`.
    #[doc(hidden)]
    pub fn new_fp(weight: Array) -> Self {
        Self {
            inner: EmbeddingImpl::Fp { weight },
        }
    }
    ...
    pub fn dense_weight_on(&self, target: impl Into<StreamOrDevice>) -> Result<Array> {
        let target = target.into();
        match &self.inner {
