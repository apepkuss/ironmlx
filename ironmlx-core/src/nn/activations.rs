...
pub fn gelu_tanh(x: &Array, target: StreamOrDevice) -> Result<Array> {
...
pub fn build_swiglu() -> CompiledFn {
    compile(
...
pub fn invoke_swiglu(func: &CompiledFn, gate: &Array, up: &Array) -> Result<Array> {
    let mut outs = func
        .invoke(&[gate, up])
...
pub fn build_geglu_tanh() -> CompiledFn {
    compile(
        |inputs: &[&Array]| -> mlx::Result<Vec<Array>> {
            let gate = inputs[0];
            let up = inputs[1];
...
pub fn invoke_geglu_tanh(func: &CompiledFn, gate: &Array, up: &Array) -> Result<Array> {
    let mut outs = func
        .invoke(&[gate, up])
